...
| I2C slave mode (`Wire.begin(address)`, `onReceive`, `onRequest`) | ❌ | Not supported — master mode only. Since v0.7.0 `Wire.begin(address)` stops the sketch with a message saying so (`panic()`, printed on `Serial`, red LED blinking SOS), rather than starting the bus as something else. `onReceive`/`onRequest` don't exist, so a sketch using them fails to build. r01lib has no slave-mode driver, but the MCUXpresso SDK's `fsl_lpi2c` it builds on does (`LPI2C_Slave*`, interrupt-driven, on both boards), so this is Arduino-layer and interrupt plumbing rather than a new low-level driver |
