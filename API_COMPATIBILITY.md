...
| `NUM_ANALOG_INPUTS` | ✅ | `4` on both boards: `A0`-`A3` on FRDM-MCXA153 (`A4`/`A5` are digital only), `A2`-`A5` on FRDM-MCXN947 (`A0`/`A1` unavailable there — see Analog section below). Was `6` on FRDM-MCXA153 before 0.7.0 |
...
| API | Status | Notes |
|-----|--------|-------|
| `EEPROM.read` / `write` / `update` / `EEPROM[i]` / `get` / `put` / `length` / range-for, `E2END` | ✅ | Added in v0.7.0, as a bundled library with the AVR EEPROM library's interface: `#include <EEPROM.h>`. 1024 bytes, as on UNO R3, kept across resets, power cycles and uploads. They live in the top of the MCU's on-chip flash (16KB on FRDM-MCXA153, 64KB on FRDM-MCXN947), which leaves a sketch 112KB of flash on FRDM-MCXA153. A write takes ~0.1-0.5ms; one every ~440 (FRDM-MCXA153) or ~250 (FRDM-MCXN947) takes up to ~6ms / ~11ms, and serial input arriving at full speed during it can be lost (measured at 115200 baud: ~20 / ~60 bytes). `write()` skips a byte that already holds the value, as `update()` does. A reset in the middle of a write leaves every earlier write intact, and that write either done or not if it is up to 12 bytes (FRDM-MCXA153) or 124 (FRDM-MCXN947); a longer `put()` can be left part old, part new (hardware-tested with 1000 watchdog resets per board; power loss is not tested). Not for use from an interrupt handler |
