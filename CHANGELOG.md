...
### Changed
- `Wire.begin()` now has the standard overloads: `begin()` joins as the controller at 100kHz, and `begin(address)` asks to join as a target (slave), as on every official core. This core's `begin()` used to take the SCL frequency as its argument instead, so a sketch written for a target, calling `Wire.begin(0x10)`, built cleanly and ran as a 16Hz controller, failing somewhere far from the cause. Target mode isn't supported yet, so `begin(address)` now stops the sketch with a message saying so. A value over 127 is no I2C address, so it is still taken as a frequency, and sketches written the old way (`Wire.begin(400000)`) keep working; that form is deprecated in favour of `begin()` then `setClock()`. Nothing in this repository or the NXP driver libraries used it. Checked on both boards by `examples/Arduino_compatible_API/test_Wire_begin_address`
