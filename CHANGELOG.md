...
- The timeout works on the LPI2C buses: `Wire`, and `Wire2` on FRDM-MCXN947. `Wire1` runs on I3C, and `setWireTimeout()` has no effect there. On a timeout `endTransmission()` returns `138` (`kStatus_LPI2C_PinLowTimeout` truncated to `uint8_t`), not AVR's `5`, in line with this core's other non-AVR codes. `Wire` was verified on hardware, both boards, by `examples/Arduino_compatible_API/test_Wire_setWireTimeout`. It needs only two jumper wires per bus: the sketch holds SCL low from a spare pin in the middle of a transfer. `Wire2` is checked by the same sketch on FRDM-MCXN947, with two more jumpers. The same sketch measures the cap at 400kHz, and on FRDM-MCXA153 it matches the numbers above; the cap at 1MHz there, ~10.9ms, was measured too. It also checks that a board that is its own target keeps answering through a timeout, with and without the reset
...
- `Serial.begin(baud, config)`, for devices that talk 7E1, 8N2 and the like (Modbus, older instruments). `SERIAL_7N1` through `SERIAL_8O2` are defined, with the same values as ArduinoCore-API. The LPUART can't do 5 or 6 data bits, mark/space parity or 1.5 stop bits, so those constants are left undefined: a sketch that asks for one fails to build, naming the constant, instead of talking to its device in the wrong format. A byte that fails the parity check is dropped, as on AVR. Underneath, r01lib's `Serial` gains Mbed's `format(bits, parity, stop_bits)`. A loopback can't tell a wrong format from a right one, since the receiver expects whatever its own transmitter sends, so `examples/Arduino_compatible_API/test_Serial1_format_end_serialEvent` also reads every frame bit by bit off the RX pin, through the GPIO input register, checking the data bits, the parity bit and the stop bits of all twelve formats, and sends hand-made frames with a wrong parity bit to see them dropped. It passes on both boards, and fails when `format()` is broken on purpose (even and odd swapped, 2 stop bits ignored)
- `Serial.end()`: waits for pending output, stops receiving, drops what wasn't read, and hands the TX/RX pins back as plain GPIO inputs. `begin()` starts the port again. On FRDM-MCXN947, where `Serial1` shares its pins with `Wire1`, ending a `Serial1` that was never begun leaves the pins alone
- `serialEvent()` and `serialEvent1()` are called after each `loop()` while their port has something to read, as on AVR. A sketch could define them before, but nothing called them
- `Wire` is now a `Stream`, as in the official cores, so it has `peek()`, `flush()`, `print()` and the `readBytes()`/`parseInt()` family, and can be passed to code that takes a `Stream&` or `Print&`. Along with it, `Print` gets a virtual `flush()` and `Stream` the `uint8_t*` forms of `readBytes()`/`readBytesUntil()`, both as in ArduinoCore-API
- I2C target (slave) mode on `Wire`: `Wire.begin(address)`, `onReceive()`, `onRequest()`, and `write()`/`available()`/`read()` inside those handlers, as on AVR. The board stays a controller too, as on AVR, and can even address its own target. Built on the SDK's `LPI2C_Slave*` interrupt API, with what it leaves to the caller filled in, each seen on hardware:
  - FRDM-MCXA153's SDK (`fsl_lpi2c` 2.5.4) switches the master off when it starts the slave, so every controller transfer after `begin(address)` waited forever. FRDM-MCXN947's SDK (2.2.4) doesn't. The master is switched back on
  - When a controller reads more than `onRequest()` wrote, the SDK sends nothing and holds the bus with clock stretching for good. The rest is sent as 0xFF, as on AVR
  - A write longer than the 128-byte buffer is NAKed from the byte that doesn't fit, with the byte still read out, since an unread byte leaves the interrupt firing
  - What a target receives is kept apart from what `requestFrom()` reads. Sharing one buffer, a board reading its own target lost the first byte, since the target's `onReceive()` ran in the middle of the read
  - Re-creating the controller (`end()`, or the reset after a `setWireTimeout()` timeout) stops the LPI2C, target side included, so the target is started again with it

  Not on `Wire1`, which runs on the I3C peripheral, nor on FRDM-MCXN947's `Wire2`: its LPI2C3 never sees the bus as a target (its slave status stays 0 through a transfer addressed to it), though it works as a controller and every slave register matches `Wire`'s LPI2C2, which works. The cause wasn't found. `begin(address)` on either stops the sketch with a message saying so. Checking this also gave `Wire2` its first run on hardware as a controller, jumpered to `Wire`: writes, reads and a bus scan all work. Checked on both boards by `examples/Arduino_compatible_API/test_Wire_target_self` (the board as its own target, no wiring, also folded into `examples/release_check/01`) and `test_Wire_target_two_boards` (FRDM-MCXA153 and FRDM-MCXN947 wired together, each the other's target in turn; also `examples/release_check/24`). Two plain examples to wire to each other and watch, one board each: `examples/Arduino_compatible_API/Wire_controller_demo` and `Wire_target_demo`
- `SDA` and `SCL`, the bare pin names every official core has alongside `PIN_WIRE_SDA`/`PIN_WIRE_SCL` (which this core already had), as constants as in AVR's and UNO R4's `pins_arduino.h`. A library that calls `pinMode(SDA, INPUT)` failed to build without them; `FXLS89xx_Arduino`, for NXP's FXLS89xx accelerometers, is one
- `Wire.requestFrom(address, quantity, iaddress, isize, sendStop)`, AVR's five-argument form, which writes a register address and reads after a repeated start in one call. Unlike AVR, it returns 0 without reading if the target doesn't acknowledge the register address

  `Serial.end()` and `serialEvent1()` are checked by the same Serial test. The `Wire` additions and fixes below are checked by `examples/Arduino_compatible_API/test_Wire_Stream_requestFrom5`, against the on-board temperature sensor on `Wire1`. Both pass on both boards, and both are folded into `examples/release_check/11` and `01` respectively

### Changed
- `Wire.begin()` turns on the internal pull-ups of `Wire`'s and `Wire2`'s pins, as AVR's `twi_init()` and UNO R4's `begin()` do, and `Wire.end()` turns them off again, as AVR's does. FRDM-MCXA153 has no pull-up resistors on D18/D19, so a bus with nothing else pulling it up floated: the first transfer after `begin()` found it busy (`132`), and so did a board writing to its own target right after `begin(address)`. Measured on hardware: on FRDM-MCXA153, `begin()` then a transfer at once gave `132` before and a NAK (`134`, nothing at that address) after; FRDM-MCXN947's D18/D19 read high either way. Next to external pull-ups the internal ones change nothing that matters, since the external ones set the rise time. The target-mode examples and tests set the pull-ups through the PORT registers themselves until now; they no longer need to. `Wire1` runs on the I3C peripheral and is left as it was
- `Wire.begin()` now has the standard overloads: `begin()` joins as the controller at 100kHz, and `begin(address)` asks to join as a target (slave), as on every official core. This core's `begin()` used to take the SCL frequency as its argument instead, so a sketch written for a target, calling `Wire.begin(0x10)`, built cleanly and ran as a 16Hz controller, failing somewhere far from the cause. `begin(address)` now does that (see target mode above), or, on a bus that can't be a target, stops the sketch with a message saying why. A value over 127 is no I2C address, so it is still taken as a frequency, and sketches written the old way (`Wire.begin(400000)`) keep working; that form is deprecated in favour of `begin()` then `setClock()`. Nothing in this repository or the NXP driver libraries used it. Checked on both boards by `examples/Arduino_compatible_API/test_Wire_begin_address`, which also checks that `Wire1.begin(address)` stops with its message

### Fixed
- With `setWireTimeout()` in effect, a transfer started right after `begin()`, `setWireTimeout()` or `setClock()` failed as busy (`132`), and the same transfer a moment later went through. So `Wire.begin(); Wire.setWireTimeout(...);` followed at once by a sensor read, the usual AVR sequence, failed its first read. The timeout also turns on the LPI2C's bus-idle detection, and with it on, a controller that has just been switched on counts the bus as busy until it has seen both lines high for that period, up to ~0.34ms. Those calls now wait out that period before returning. Found while checking target mode against timeouts; `examples/Arduino_compatible_API/test_Wire_setWireTimeout` (and `release_check/14`) had missed it, since every transfer there came after a `Serial.flush()` long enough to cover the wait. It now also sends one at once after each call, and fails against the old code
- A `panic()` never showed its message. Every one of this core's hard stops (an unsupported pin for `Serial`, `analogRead()` or `analogWriteFrequency()`, and now `Wire.begin(address)`) calls `panic()`, which printed through `PRINTF`, but the core builds with the SDK debug console off, so `PRINTF` became `printf`, whose output the core discards. All anyone saw was the red LED blinking SOS. `panic()` now writes `error: <message>` straight to the USB serial port, where the Serial Monitor is, by polling the UART: it works before `setup()` (a panic during static initialization), without `Serial.begin()`, and from an interrupt handler. What the sketch printed just before is sent out first, and the message repeats on every round of the blink, so a Serial Monitor opened late still gets it. Checked on both boards, for a panic in `setup()`, one during static initialization, and one with `Serial` never begun. The 0.6.0 entry below saying `analogWriteFrequency()` "stops with a clear message" was true only of the LED until now
- `Wire.write()` never checked its 128-byte buffer. The 129th byte of a transaction went into whatever came after the buffer, starting with the count of bytes queued so far, and `requestFrom()` of more than 128 bytes stored what it read past it the same way. `write()` now returns 0 once the buffer is full and sets the write error, as on AVR, and `requestFrom()` cuts a longer request down to 128 bytes, as AVR cuts one down to its 32. `write()` also returned the total queued so far rather than the bytes it just queued, so `n += Wire.write(x)` counted wrong. It returns 1 now
- `Wire` kept outgoing and incoming bytes in one buffer, so a `beginTransmission()` before everything `requestFrom()` got had been read made `available()` and `read()` hand back the wrong bytes. They have separate buffers now
- `Serial.write(0)` failed to build (`call of overloaded 'write(int)' is ambiguous`): `0` converts to `uint8_t` and to `const char*` equally well. `Serial` and `Wire` now have the `write(int)`-style overloads AVR has, which send the low byte
- Uploading failed whenever more than one board was plugged in, even with a port selected: the upload scripts never passed the port on, so LinkServer saw several probes and stopped with `Multiple probes detected`. The selected port's USB serial number is the probe's own serial number as LinkServer lists it, so `platform.txt` now hands it to `upload.sh`/`upload.bat`, which pass it on as `--probe` when the port is an NXP probe (VID `0x1FC9`). With no port selected, or a port that isn't an NXP probe, the scripts behave as before. When the upload fails that way, they now also say to select the board's port, since LinkServer's own message asks for a `--probe` option an IDE user can't pass. Checked on macOS with FRDM-MCXA153 and FRDM-MCXN947 both attached, uploading to each in turn from Arduino IDE 2 and with `arduino-cli upload -p`. Not yet checked on Windows and Linux, which report the serial number through their own port discovery.
- Debugging had the same problem with more than one board plugged in: `gdb-bridge` started LinkServer's gdbserver without a probe, so it refused to start. The port can't be used here the way uploads use it, because Arduino IDE 2 doesn't pass the selected port when it asks for the debug configuration. `gdb-bridge` now runs `LinkServer probes`, which lists the chip each probe is wired to, and picks the one on the board's own chip (`MCXA153` or `MCXN947`). That tells an FRDM-MCXA153 from an FRDM-MCXN947, not two boards of the same kind. For two of the same kind it now stops with a message saying so, instead of LinkServer's advice to pass `--probe`, which no IDE user can do. Checked on macOS with both boards attached, on both launch paths: `arduino-cli debug` and the way the IDE's cortex-debug starts it. The table parsing and each outcome are covered by a new `go test` in `tools/gdb-bridge/src/`. All five `gdb-bridge` binaries are rebuilt. Also checked with the Debug button in Arduino IDE 2 on macOS, on each board with both attached. Not yet checked on Windows or Linux
