...
- `Wire1`（オンボードI3Cセンサーバス、MB_RX/MB_TX＝P1_16/P1_17、I3CをI2C_MODEで
  ...
  2. **I2C_MODEに切り替える前に一度も実I3Cバス動作をしていないと、以降の
     ...
     不要**（理由は確かめていない。以前ここに「A153のI3C_SDA/I3C_SCLはD18/D19と
     共有で実プルアップ抵抗があるため」と書いていたが誤り——A153のI3Cは
     `P0_16`/`P0_17`、D18/D19は`P1_8`/`P1_9`で別のピン。N947のオンボードI3Cバスは固定プル
...
- **実機バグ発見・修正: `MB_RX`/`MB_TX`だけ出力が出ない**: MikroBus
  ...
  抱えていたことが判明。`arduino_layer/arduino_io.cpp`（現在は`cores/arduino/arduino_api/arduino_io.cpp`）の`pinMode()`を
...
- **`Wire2`はコントローラとしては動くが、I2Cターゲットにはなれない**:
  `Wire`（LPI2C2）とジャンパでつなぎ、書き込み・読み出し・バススキャンが
  通った（`Wire2`のコントローラとしての初めての実機確認）。一方、ターゲット
  としては、自分宛てのアドレスが来てもLPI2C3のスレーブステータスが0のまま
  で、スレーブ側のレジスタは動いている`Wire`とすべて一致していた。原因は
  見つかっていない。`Wire2.begin(address)`はその旨のメッセージで止まる
- **`setWireTimeout()`は`Wire2`でも効く**: `test_Wire_setWireTimeout`
  （`release_check/14`）で、`MB_SCL`-`MB_PWM`＋`MB_SDA`-`MB_INT`のジャンパを
  使って確認。上限はどのバス速度でも約87ms
- **`EEPROM`はフラッシュの第2バンクの末尾64KB**: 1回の書き込みは128バイト
  のレコード（このフラッシュが一度に書ける最小単位）。SDKの`fsl_flash.c`を
  無改変で使う。書き換え中は割り込みは止まらないが、書き込みが終わるまで
  スケッチが待つので、長い書き込み（約11ms）の間に受信が続くと`Serial`の
  64バイトの受信バッファがあふれる（115200bpsの連続受信で約60バイト欠落）
- **消去・書き込みの途中でリセットされたフラッシュは、読むとバスフォールトに
  なる**: ECCが合わなくなるため。`EEPROM`は以前、その次の起動で最初に読んだ
  所でハングしていた（アップロードではこの領域を消さないので、以後毎回）。
  今はフォールトを止めて読み、読めない所は使っていない半分・途中で切れた
  レコードとして扱う。ウォッチドッグで書き込み中に1000回リセットした試験で、
  245回の起動が読めないフラッシュを見つけ、すべて正しく続いた（A153では
  2000回で一度も起きていない）
- **2枚つないだままのデバッグ**: gdb-bridgeは`LinkServer probes`の`Device`列
  が`MCXN947`のプローブを選ぶ。macOS・Windows・Linuxで確認
