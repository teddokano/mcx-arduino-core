...
**スケッチが止まり、赤LEDがSOS（`・・・ ─── ・・・`）で点滅する場合:**
- コアが意図的にスケッチを止めています。理由は、シリアルモニタに`error:`で始まる行として数秒ごとに繰り返し表示されます（ボーレートはスケッチの`Serial.begin()`の値、`Serial.begin()`を呼んでいなければ115200）
- `error: HardFault: ...`は、プログラム自体がクラッシュしたことを示します。不正なポインタや、スタックあふれ（ローカル変数の使いすぎや、終わらない再帰）などです。その行の`PC 0x...`が発生箇所です。ソースの行に直すには、ビルドで作られた`.elf`（**スケッチ → コンパイルしたバイナリをエクスポート**でスケッチのフォルダ内の`build/`に出力されます）に対して`arm-none-eabi-addr2line -e <スケッチ>.elf 0x...`を実行します
...
数字を入力すると、少し遅れて`got: 0`という行がもう一度出ることに気づくかもしれません。これはバグではなく`parseInt()`の一般的な仕様です——シリアルモニタが送るのは`42\n`のように数字＋改行で、`parseInt()`は数字部分だけを消費して改行は読み捨てずに残します。次の`loop()`でその残った改行が読み捨てられた後、新しい入力が来るまで（デフォルトのタイムアウトである）約1秒待ってから`0`を返す、という動作です。UNO R3など従来のArduinoでも同じ挙動になります。
...
同梱の`EEPROM`ライブラリは、UNO R3と同じ1KB・同じインターフェースを、MCU内蔵のフラッシュの上位に用意します。保存した内容は、リセットしても、電源を切っても、新しいスケッチを書き込んでも残ります。次の例はボードが起動した回数を数えます:
