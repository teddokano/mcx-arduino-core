...
**検証**: 両ボード`--warnings all`で警告ゼロ（A153 65004B / N947 67948B）、hygiene全項目パス（新設したコメントブロックの`*/`早期終了チェック含む）。**`--library`指定なしでコンパイルが通った**ことで、同梱プラットフォームライブラリが`release_check/`配下のスケッチからも正しく解決されることも確認できた。**実機確認は未実施**——`PWM0`-`D8`の5本目のジャンパを足して両ボードで通すこと。
