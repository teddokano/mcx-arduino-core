...
- **実バグ2件**（未修正）: (1) `arduino_spi.cpp`のバルク`transfer(buf, size)`が128バイトのスタックバッファへ`alloca`的にコピーする実装になっており、それを超えるサイズの転送でスタックオーバーフローの恐れ。(2) `arduino_i2c.cpp`の`TwoWire::requestFrom()`が`stop`引数を受け取りながら実装内で無視しており、リピートスタート制御ができない
