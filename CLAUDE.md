...
- **`test_SPI_large_transfer`の実機検証: 進行中**。ユーザーから「データを連番にして何バイト目かをわかりやすくして」と改善依頼があったため着手しかけたが、直後に「問題なかった．ロジアナのGNDが外れていて，データがおかしかっただけ」と訂正——見えていた異常はスケッチ側の問題ではなく、ロジアナのGND未接続によるプローブ側の計測不良だった。ただし検証作業自体はまだ完了しておらず継続中（GND接続し直した上での本来の確認はこれから）
