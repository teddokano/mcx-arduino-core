...
**「A153だけが潰していた」の中身**: 修正前のA153分岐は、A156と同じ形のif/elseチェーンを持ちながら**全ての枝の中身が空の`;`**で、その後に`constexpr int mux_setting = kPORT_MuxAlt3;`と**4ペア分のALTを定数1つに畳み込んで**いた。つまりチェーンがピンの妥当性検査だけをやって結果を使っていない状態。A153はLPI2Cが1系統しかないので`unit_base`は本当に1つで正しいが、**ALTまで一緒に「1つでいい」と扱ってしまった**のが誤り。A156・C444・N947はどれもペアごとに`mux_setting`を代入する形になっており、A153だけがこの形だった。
