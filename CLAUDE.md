...
- **優先度低のコード指摘**（未対応）: `I2C::scan()`の`start`引数が無視されている＋走査範囲のoff-by-one疑い、`BusIn`/`BusOut::config()`内の`static_assert`が実質no-opになっている、`I3C`コンストラクタ内で`_scl`/`_sda`のローカル変数名が入れ替わっている（値の代入自体は正しいが可読性上の罠）
...
- **`test_SPI_large_transfer`の実機検証完了**: ユーザーから「データを連番にして何バイト目かをわかりやすくして」と改善依頼があったため着手しかけたが、直後に「問題なかった．ロジアナのGNDが外れていて，データがおかしかっただけ」と訂正——実際にはスケッチ側に何の問題もなく、ロジアナのGND未接続によるプローブ側の計測不良だった。スケッチの変更は不要、そのままで実機検証完了。**これで監査由来の実バグ2件（SPIチャンク分割・`I2C::read_core()`の`stop`無視）とも実機検証済みとなった**
