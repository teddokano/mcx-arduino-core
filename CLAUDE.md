...
### GPIOループバックジグ（`gpio-loopback-jig`ブランチ・0.6.0には入れない）
「`02`のGPIOウォークをロジアナ目視ではなくループバックで自動判定にしたい、配線はシールド基板のジグで用意する」というユーザー提案。**0.6.0は現状の`02`のウォークのまま出す**と決めたうえで、設計だけ専用ブランチで進めたもの。
...
**未着手・未確認**:
- `31_gpio_loopback`のスケッチ本体、`02`→`02`＋`09`の分割、README/チェックリスト更新
- **A153の`Wire1`（`P0_16`/`P0_17`）がジグから届くか**——Arduinoヘッダにもウォーク対象にも無い独立ピンなので、ロジアナCH12-13をA153でも使えるかは`ref/FRDM-MCXA153.pdf`次第
- I2C/I3Cラインに付くGPIOスタブ（A153の`D18`-`A0`/`D19`-`A1`、N947の`D13`-`D18`/`D19`-`MB_CS`/`MB_PWM`-`MB_SCL`/`MB_INT`-`MB_SDA`）は、400kHz I2Cなら無視できるがI3Cのプッシュプル12.5MHzでは基板上のスタブ長を短くしておきたい
