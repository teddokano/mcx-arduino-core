...
4. **v0.3.0リリース**: バージョン番号は更新済み（`platform.txt`の`version=0.3.0`、`package_nxp_mcx_index.json`の`platforms`配列に新エントリ追加——`boards`は両ボードの新表示名`FRDM-MCXA153 (mcx-arduino-core)`/`FRDM-MCXN947 (mcx-arduino-core)`で記載、checksum/sizeは旧バージョンの値のままの仮置き、ローカル開発用symlinkも`0.3.0-dev`に改名）。残るは実際のリリース作業——これまでと同じ手順（リリースzip作成→GitHub Release作成→タグpush→`update_package_index.yml`の自動push失敗を確認→`main`に対して`workflow_dispatch`を手動実行してchecksum確定）。リリースzip作成時は単一トップレベルディレクトリでラップすることを忘れないこと（v0.2.1初回リリースの教訓）
