...
- **今回のセッションの本題（ピンリナンバリング修正）としては、A153側で完全に実証されたことで検証完了**——N947のネイティブI3C SDR不具合は、今回のピン修正とは無関係の別問題と切り分け済み。ユーザー判断で**既知の課題として保留**し、ソース化作業（`cores/arduino/`一本化）に戻ることに決定（**後日、根本原因を特定して修正済み——下記「保留していたN947ネイティブI3C SDR不具合の原因特定」セクション参照**）
...
### 保留していたN947ネイティブI3C SDR不具合の原因特定: `I3C1_PUR`(P1_11)のピンmux漏れ（実機検証待ち）
...
- **修正**: `i3c.cpp`のコンストラクタ、既存のIBE回避策の直後に`#ifdef CPU_MCXN947VDF`で`DigitalInOut _pur( P1_11 ); _pur.pin_mux( kPORT_MuxAlt10 );`を追加（既存のSDA/SCLと同じ`DigitalInOut`ローカル＋`pin_mux()`パターン、PORTレジスタ設定なのでローカル変数のスコープを抜けても設定は残る）。両ボードでコンパイル確認、全55サンプル×両ボード回帰スイープも新規失敗なし（既知の11件のみ）
- **副次的な予測（未検証）**: これが正しければ、`TwoWire::begin()`のN947限定RSTDAAプライミングハック（「I2C_MODEに落とす前に一度I3Cバス動作をしないと以降のreadが必ずNAKする」、長らく根本原因**未特定**として`#ifdef`で囲んで残していたもの）も同じ原因だった可能性が高い——I3Cモードのトランザクションを試みることでペリフェラル内部のプルアップアシストが有効化され、それがI2Cモードに持ち越されていた、と考えると2つの謎が単一の原因で説明できる。**PUR修正後にこのハックを外しても`Wire1`が動くなら、仮説の強い裏付けになる**（実機検証時に併せて確認する価値あり）
- **実機検証は未実施**（次のステップ）
