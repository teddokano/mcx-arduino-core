...
- 普段は読む必要がありません。`CLAUDE.md`が自動で読み込まれるのに対し、
  こちらは**必要になったときだけ開く**運用です（`CLAUDE.md`を軽く保つための分割）。
- 再発防止の教訓は`CLAUDE.md`の「繰り返し踏んだ罠と教訓」に要約済みです。
  まずそちらを見て、背景・当時の切り分け手順を知りたいときにこのファイルの該当節へ。
- 過去のセッションが作った記述なので、**後から訂正された箇所があります**。
  該当箇所には`【<日付>訂正】`と明記してあるので、訂正注記を必ず一緒に読むこと。
- `examples/release_check/`の番号は0.5.0で振り直しています。それより前の記録に出てくる
  旧番号（`04`/`05`/`06`/`09`/`0A`/`0B`/`0C`）は、v0.5.0の節にある新旧対応表で読み替えること。

---

## v0.1.x〜v0.3.2 期の概要記録（元は`CLAUDE.md`冒頭の「プロジェクト概要」節）

`CLAUDE.md`を軽くするため、当時の経緯を述べたこの塊をこちらへ移した。
**「現在のバージョン」は移設時点で既にv0.3.2のまま5リリース分古かった**ので、
現状は`CLAUDE.md`側の新しい「プロジェクト概要」を見ること。ここは当時の記録として読む。
...
- **根本原因**: `pin_mux.c`の778行目に`/* PORT1_11 (pin D3) is configured as I3C1_PUR */ PORT_SetPinMux(PORT1, 11U, kPORT_MuxAlt10);`があるが、これは`BOARD_InitDEBUG_UARTPins()`の中にある。**【2026-09-10訂正】当時「この関数はどこからも呼ばれていない」を前提にしていたが誤り**——実際は`BOARD_InitBootPins()`経由で呼ばれており、**P1_11は既にALT10へmuxされていた**。つまりこの仮説で追加したコードは最初から冗長で、これが「実機で症状が全く変わらなかった」ことの直接的な説明になる（下記のI3C仕様上の理由と合わせて二重に不要だった）
...
### `examples/release_check/`実機検証: 両ボードとも`23`を除き完了
新番号体系（`0n`/`1n`/`2n`）確定後、N947→A153の順に実機で通しで実施。

- **N947**: `01`〜`05`（配線不要グループ）・`11`〜`13`（ジャンパのみ）・`21`（複合ペリフェラル、外部`P3T1755.h`＋MikroBus MOSI-MISO配線——A153と違いN947は`Serial1`が使えないため`Wire2`バスプローブで代替、`wire2Err=134`＝デバイス未接続への期待どおりのNAK）・`22`（外部LM75系センサー、`SENSOR_ADDR=0x72`のまま、D18/D19/3V3/GND）まで全て実機確認済み。`21`・`22`ともWARNING行なし、`22`はロジアナで`Write[0x72]→0x00+ACK→Read[0x72]→0x1D+ACK→0x90+NAK`の正しいシーケンスを確認
- **A153**: `01`〜`04`（`05`はN947限定のためスキップ）・`11`〜`13`・`21`（A153はD1-D0＋MikroBus MOSI-MISO配線、`Serial1`ループバック`hb0`〜込みの完全版）・`22`（D18/D19/3V3/GND、ロジアナで`Write[0x72]→0x00+ACK→Read[0x72]→0x1C+ACK→0x50+NAK`を確認）まで全て実機確認済み
- **これで今回のリリースで修正した3件のI2Cバグ（`Wire`/`Wire2`のNAK後STOP欠落・グリッチフィルタ残留、`Wire1`のOD分周スケーリング）が、実デバイス通信・複合ペリフェラル同時稼働・ループバックの全パターンで両ボード実機確認が取れた**
- `21`/`22`は外部`P3T1755.h`ライブラリが必要なため、CIスタブではなく実物（`~/dev/Arduino/libraries/TempSensor_NXP_Arduino`＋依存の`I2C_device_Arduino`）を`--library`指定してコンパイル・書き込み
...
### 将来構想として調査した「A153ネイティブUSBでSDカードをPCにストレージとして見せるブリッジ」（0.5.0では見送り）
ユーザーから「A153のネイティブUSBを使う手を考える。MCUXpresso SDK内のTinyUSBが使えるか。やろうとしているのはSDカードをPCにストレージとして見せるブリッジ。どの程度の労力でできそうか」と相談。実現可能性を実際に確認したうえで、**このバージョンでは見送り**とユーザーが判断。調査結果は再調査を避けるため以下に記録する。

**ハードウェアは条件を満たしている**（回路図4ページ目を画像で視認確認）:
- MCXA153にネイティブUSB（`USB0`、フルスピードのデバイスコントローラ、オンチップPHY付き）があり、レジスタ定義・クロック供給の仕組み（`kCLK_48M_to_USB0`、`kCLOCK_GateUSB0`）も揃っている
- FRDM-MCXA153基板に**ターゲットMCU用のUSB Type-Cコネクタ（J8）**が実装済みで`USB0_DP`/`USB0_DM`が配線され、VBUS検出は`P2_12`（`USB0_VBUS_DET`）。MCU-Linkのデバッグ用USBとは別系統——**未配線テストポイントではない**ことを確認済み（analogWriteピンの前例があるため必ず視認確認すること）

**SDKの収録状況**（`ref/SDK_2_16_100_FRDM-MCXA153_usb.zip`、ユーザーがUSB込みで再生成して配置）:

| 項目 | 状況 |
|---|---|
| NXP USBミドルウェア | あり（`middleware/usb/`） |
| MSCクラスドライバ | あり（`usb_device_msc.c/.h`、`usb_device_msc_ufi.c/.h`） |
| CDC vcomサンプル（本ボード向け） | あり |
| **MSCサンプル（本ボード向け）** | **無し** |
| TinyUSB | **無し**（使うなら上流から取り込み） |

- **重要**: SDK Builderで生成する際にUSBを選択しないとUSBが一切入らない。最初にユーザーが配置した`SDK_2_16_000_FRDM-MCXA153.zip`は`middleware/bm/`のみで`fsl_usb*`ドライバもUSB例も無かった。26.06.00版を経て、最終的に2.16.100のUSB込み版で確定
- **バージョン整合の確認手法と結果**: リポジトリにvendorされているA153ドライバとSDK zip内のファイルをバイト比較（N947のときにMD5一致で特定したのと同じ手法）。`fsl_clock.c`（チップ・バージョン依存が最も強い）は**完全一致**。`fsl_lpi2c.c`/`fsl_gpio.c`の差分は計3箇所で、いずれも`MSDK_REG_SECURE_ADDR()`によるアドレス正規化の有無のみ——TrustZone対応のためのもので非TrustZoneビルドでは実質no-op。**機能的な非互換ではない**
- **TinyUSBについて**: SDKには入っていないが、**上流のTinyUSBにFRDM-MCXA153のポートが存在**し、マージ済み・コンプライアンステスト通過との情報あり（NXP Community / wavenumber.netの記事）

**前提の訂正2件**（ユーザーの当初の想定と実態が違った点）:
- **`Waveshare_TFT_Touch`にSDドライバは含まれていない**——`src/`にあるのは`ST7789`と`XPT2046`のみ。`SDBitmapViewer`は標準の`SD.h`（Arduino公式SDライブラリ）を使っている。ユーザー確認済み、Arduino公式SDライブラリを使う方針で、CS等の信号はWaveshare_TFT_Touchと同じ配線を流用
- **USB MSCにはセクタ単位のブロックアクセスが必要**で`SD.h`のファイルAPIでは足りない。`Sd2Card::readBlock()`/`writeBlock()`を使うことになる

**リソース余裕は問題なし**（当初の最大の懸念だったが実測で否定）: `SDBitmapViewer`（SD+TFT+タッチ）で70KB/128KB・RAM 5.9KB/24KB。MSCブリッジならTFT/タッチ不要なので、コア43.6KB + SD約15KB + USB/MSC約12KB ≈ 70KB程度、RAMも10KB弱の見込み

**着手する場合の推奨手順**（不確実性の高い順に潰す）:
1. **まずCDC vcomをこのコア上で動かす**——NXPのCDCサンプルをArduinoのビルドに載せ替え、PCから仮想シリアルとして見えるところまで。見積もりで最も不確実とした部分（USB0のクロック供給48MHz・NVIC結線・`usb_device_khci`コントローラドライバの初期化・VBUS検出）はすべてCDCサンプルに動く形で入っているので参照できる
2. **次にMSCへ差し替え**——クラスドライバは公式のものを使い、ディスクリプタと`USB_DeviceMscCallback`等を自力で書く（本ボード向けMSCサンプルが無いため）。ディスク実体は最初RAMディスクで動作確認し、その後`Sd2Card`に繋ぎ替える。段階を分けることで、失敗時にUSB側かSD側かを常に切り分けられる

**残るリスク**: (1) TinyUSBは独自ビルドシステム前提なのでArduinoの「`cores/`配下を全部コンパイル」方式への適合に手間がかかりうる（NXPスタックを使うなら回避可能）、(2) スループット——SD over SPIなのでUSB FSの12Mbpsに対しSPI側が律速になるはず。「PCから見えて読み書きできる」は達成できても実用速度かは実測しないと不明

**規模感**: ボード追加に近い、単独で1バージョン分の作業量。0.5.0のリリースを済ませてから着手するのが安全（USBはコアの構造——`platform.txt`のビルド設定・`mcu.cpp`のクロック初期化——に手を入れるため、検証済みの成果と混ぜない）
...
### 実際のGitHub Actions実行での検証（3回、実バグ1件発見・修正）
`0.5.0-dev`にpushして実runnerで検証。

- **1回目: 失敗**。`arduino-cli`のインストールステップ（`curl ... install.sh | sh -s -- -b /usr/local/bin`）が、install.shの引数解釈ミスにより`arduino-cli_-b_Linux_64bit.tar.gz`という壊れたURL（`-b`という文字列がバージョン部分に混入）をダウンロードしようとして404失敗。ローカルではこのインストール手順自体を検証していなかった（ローカルは既にarduino-cliがインストール済みの環境だったため気づけなかった）ため、実runnerで初めて発覚。自前のinstall.sh呼び出しをやめ、GitHub公式の`arduino/setup-arduino-cli@v2`アクションに置き換えて解消
- **2回目: 成功、両ボードとも所要時間が1回目よりむしろ増加**（A153: 1m26s→2m03s、N947: 2m07s→2m18s）。キャッシュヒット自体は両方（ツールチェーン242MB・ビルドキャッシュ58MB）確認できたため、キャッシュ設計自体の不備を疑い、ログのタイムスタンプを解析——原因は`actions/checkout@v4`内部の`git fetch`が52秒かかっていたこと（通常は1〜3秒）。ビルドキャッシュ自体は正しく機能していた（1本目のコンパイルこそ14.8秒だが、2本目以降は2〜4秒台で安定——ローカル計測の「温かい」速度と一致）ことも、各`::group::`のタイムスタンプ差分から確認済み
- **3回目: 成功、1回目相当の速さに復帰**（A153: 1m21s、N947: 2m10s）、`git fetch`も1.5秒/2.9秒に戻った。**2回目の52秒はGitHub側の一過性のジッターであり、CI設計側の問題ではないと確定**——同一コミットからの3回のfetchのうち1回だけ異常値が出たことと、その異常がキャッシュ設計と無関係な`checkout`ステップ単体で完結していたことから判断
...
**副産物として見つかった`Wire1`（I3C in I2C_MODE）の別系統バグ——同セッション内で修正・実機検証済み**。`Wire1.setClock()`が最初の1回しか効かず、400kHz要求→実測416kHz（正常）に対し100kHz要求→2.5MHz、10kHz要求→1.25MHzという無関係な値になっていた。原因は`TwoWire::setClock()`がI3Cに対し`frequency(baudrate, 0, 0)`とオープンドレイン側を据え置いて呼ぶこと——I2Cモードのレートは`I2CBAUD`（3ビット幅、`i2cFreq = odFreq/(I2CBAUD+1)`）でしか作れないため、ODを4MHzに固定したままでは4MHz/8より下が到達不能になり、それ以下を要求すると切り詰めて高い周波数になる。

- **ユーザーからの設計上の指摘が方針を決めた**: 「r01libのI3Cを直接使う例もあるので、I2C/I3Cのモード変更の際に厳密に切替を行う」。これによりArduino層（`TwoWire::setClock()`）ではなく**`I3C::mode()`側**に周波数ポリシーを置く形になった——I2C_MODEへ切り替える際はI2C用の分周を、I3C_MODEへ戻す際はネイティブ用（OD_FREQ=4MHz/PP_FREQ=12.5MHz）を確実に再適用する。`r01lib_I3C.ino`のように直接I3Cを使う経路を壊さないための要点
- **実装**: `I3C::apply_baudrate()`を新設してモード別の分周設定を一箇所に集約（I2C_MODEでは`od = min(i2c_freq * I2C_MODE_OD_RATIO, OD_FREQ)`、`I2C_MODE_OD_RATIO=8`）。`mode()`が双方向でこれを呼ぶ。あわせて`I3C::frequency()`が要求値を`masterConfig`へ記録するよう修正——従来は構築時の値が残り続け、後から`0`（据え置き）で呼ぶと「最後に設定した値」ではなく初期値に戻る挙動だった
- **クランプ範囲は実測ベース**: 当初レジスタ幅（ODBAUDが8ビット）から下限10kHzと見積もったが、**実機では10kHz要求が56kHzで頭打ち**だったため`I2C_MODE_MIN_FREQ = 50kHz`に訂正。レジスタ幅から導いた理論値より実測を信じる、という教訓をコメントにも残した。実測精度: 400kHz→357kHz、100kHz→104kHz、下限→約56kHz
- **修正結果**: 416k/2.5M/1.25M（逆転・破綻）→ 357k/104k/56k（単調・100kHzは正確）。実機で`test_Wire1_frequency_accuracy_N947`・`test_Wire1_onboard_sensor_raw`（実デバイスACK系）・`r01lib_I3C.ino`（ネイティブI3Cモード）の3本とも問題なしを確認

**同時に判明したが今回は見送った既知の制限（NAK後にSTOPが出ない）**。`Wire1`でアドレスフェーズがNAKされると（バススキャン等、存在しないアドレスを叩いたとき）STOPがバスに出ない。`I3C_MasterTransferBlocking()`はデータフェーズのNAKにはSTOPを出すが、アドレスフェーズは手前の`I3C_MasterWaitForCtrlDone()`で検出され、そのパスがSTOPを出さずにbare returnするため——**i2c.cpp側で直したLPI2Cのバグと全く同じ形**。ただし対応は見送った:
- **切り分けの手順として、まず変更を全て`git stash`して素の状態で実機確認し、これが私の変更が引き起こしたものではなく既存バグであることを確定させた**（ベースラインを取らずに修正を重ねていたことへの反省。ユーザー承認のうえ実施）
- `I3C_MasterStop()`を後追いで呼ぶ修正を試したが、**STOPの手前に余計なRepeated-STARTが乗る**波形になり、正しく直せていないと判断してリバート。I3Cコントローラの状態機械の理解が要る領域で、この経路では既に2回外していたため深追いを打ち切った
- **実用上の影響は限定的**: NAKが起きるのは存在しないアドレスを叩くときだけで、`Wire1`本来の用途（オンボードセンサー、ACKが返る）では`test_Wire1_onboard_sensor_raw`でSTOPが正常に出ることを実機確認済み
- **この経路での私の失敗2件も記録**: (1) `xfer()`が`#ifdef CUSTOM_REGISTAR_XFER`で2つ定義されており、実際にビルドされない`#else`側を修正して「効かない」と誤認した（実体は`reg_xfer()`）。(2) `i3c.h`はLF・`i3c.cpp`はCRLFと**同一ディレクトリ内でファイルごとに行末が異なる**ため、Pythonでの置換が一度失敗した
- **後日の再検討で判明した重要な事実**: ユーザーから「Wire1をもう一度検討」と依頼を受けて掘り下げた結果、**NAK後にSTOPを追加する修正は正常系を壊す**と分かった。I2Cのリードは最後のバイトでマスターがNAKを返して終わるのが正常な作法だが、呼び出し側で`I3C_MasterTransferBlocking()`の戻り値だけを見ると**アドレスNAKとリード終端のNAKが区別できない**ため、実デバイスからの正常な読み出し（`test_Wire1_onboard_sensor_raw`、0x48のセンサー）にまで余計なSTOP発行が走り、rSTARTが付いてしまった。**SDKがこの処理を呼び出し側に置いていない理由の説明にもなっている**——SDKが末尾でNAK時にSTOPを出すのは、そこに到達する時点で「データフェーズのNAK」と文脈が確定しているからで、戻り値だけで一律に判断すると文脈を失う。ユーザーの指摘で全面リバート
- **さらに重要**: リバート後もrSTARTが残ったため、**Wire1関連の作業を一切行っていない状態（`7417551`、LPI2C修正のみ）に戻して確認したところ、そこでもrSTARTが出た**——つまり**正常なリード終端にrSTARTが付くのは元からのコントローラ挙動**で、OD連動修正（`5b545fe`）も含め私の変更はどれも原因ではない。長年このバスでオンボードセンサーが正常動作している実績があるので実害はない。**同じ波形を見て再調査しないこと**
- **この件で私が外した仮説5つ**（記録として）: (1) ビルドされない`#else`側の`xfer()`を修正、(2) 残存`MERRWARN`が原因（実測では`MERRWARN=0`、NAKは`MSTATUS.NACKED`に立つ）、(3) `waitIdle`の値、(4) `enableOpenDrainStop`（SDKデフォルトの`true`に変えても不変、検証できなかったので元の`false`に戻した）、(5) `MSTATUS.NACKED`の事前クリア。決定打になったのは毎回レジスタ実測（`MSTATUS=0x1023`→STOP後`0x1000`）とベースライン比較であり、コードリーディングからの推測ではなかった
...
### 項目1完了: 機械的ミスの検出をCIに追加（`.github/scripts/check_repo_hygiene.py`）
0.5で予定しながら落ちていた項目。実機不要で完結する項目から着手するというユーザー方針で最初に実施。**ツールチェーン不要の独立ジョブ`hygiene`**として`regression_check.yml`に追加（数秒で完了、compileマトリクスとは独立に結果が出る）。

**5つのチェックと、それぞれが実在する再発ミスに対応している根拠**:

| チェック | 過去の実績 |
|---|---|
| `version-fields` | `platform.txt`の`version=`と`version.major`/`minor`/`patch`の齟齬。手で3行並べて保守しているため黙ってずれる（マクロが別バージョンを報告することになる） |
| `doxyfile-version` | `Doxyfile`の`PROJECT_NUMBER`更新忘れ——v0.4.1で実際に発生、生成ドキュメントに古い番号のまま入りかけた |
| `comment-terminator` | ブロックコメント内の`*/`による早期終了。**4回踏んでいる**（`GPIO_Type*/bitmask`・`ARD_D*/ARD_A*`・`D0-D19/MB_*/SPI_*/ARD_*`・`release_check/NN_*/`）。毎回「実際のミスとは似ても似つかない場所で出るビルドエラー」で発見していた |
| `mcxpinstate-aliases` | `ALIAS_NAMES[]`と`arduino_pin_by_number[]`の乖離 |
| `changelog-heading` / `package-index-entry` | CHANGELOG確定忘れ、`package_nxp_mcx_index.json`のプレースホルダー追加忘れ（v0.4.0・v0.4.1で2回） |

**`mcxpinstate-aliases`だけは「手作業をCIに移した」以上の価値がある**: `PinState.cpp`の既存`static_assert`は**両テーブルの長さしか見ていない**——ピンの追加・削除は捕まえるが、**個数が同じままの並べ替え・改名は原理的に捕まえられない**。CLAUDE.mdにも「`static_assert`では拾えない稀なケースの保険としてリリースごとに手で突き合わせる」と書いてあった、その手作業を機械化した。実際に`"MB_RX", "MB_TX"`を入れ替えて検証したところ、`static_assert`は通るがこのチェックは2件とも検出した。

**`comment-terminator`の判定方法**: ブロックコメントの状態を追跡し、**「行の開始時点で既にコメント内だった継続行」で`*/`が現れ、その後ろに非空白が続く**場合のみ検出する。`/* n */ int n;`のような単一行での途中終了は正当で頻出するため対象外にしてある。文字列リテラル内の`*/`も読み飛ばす。過去4件は全てこの形（`/** ... */`の継続行の途中）だった。

**リリース時限定チェックの扱い**: `changelog-heading`と`package-index-entry`は、開発途中のブランチでは**正当に未確定**（サイクル進行中はCHANGELOGが`[Unreleased]`または前リリースの見出しのまま、index エントリもまだ無い）。そのまま適用すると毎pushで落ちるため、`--release`フラグで分離し、**`main`とタグでのみ有効**にした。あわせて`update_package_index.yml`の冒頭にも同じ`--release`実行を追加——**エントリ欠落が実際に問題になるのはこのworkflowを走らせる瞬間**で、v0.4.0・v0.4.1ではここが「真因に一切触れないエラー」で落ちていたため、その地点で明示的に言わせるのが最も的確。

**検証方法**: 5つの不具合を実ファイル上で再現→該当チェックが発火することを確認→`git checkout`で復元、を1件ずつ実施。あわせて**正しい状態（0.5.0——確定済み見出しとindexエントリの両方を持つ）では`--release`が通ることも確認**——「常に落ちるだけのチェック」ではないことの裏付け。現状のリポジトリに対しては誤検出ゼロ（53ピンの照合も一致）。

### 項目3完了: `docs/porting_a_new_board.md`を新設
実機不要で完結する2件目。**書く過程で「ボード追加時に手で触る箇所」を実測で洗い出したことが、手順書本体と同じくらいの成果**になった。

**実測した「手で触る箇所」**（`grep`でボード分岐を全数カウント）:
- **`cores/arduino/`の共有ツリー20ファイルがボードごとに分岐している**——`variants/<board>/`を作るだけでは済まない。内訳は「ピン/インスタンステーブル」（`io.h`/`io.cpp`、`Serial.cpp`——分岐10箇所で最多、`i2c.cpp`/`i3c.cpp`/`r01lib_spi.cpp`、`AnalogIn`/`PwmOut`は**クラス定義丸ごと重複**）、「クロックとシンボル名の差異」（`mcu.cpp`、`arduino_tone.cpp`——分周関数名すら`CLOCK_SetClockDiv`/`CLOCK_SetClkDiv`と違う）、「ペリフェラルの有無」（`arduino_i2c.h`の`Wire2`は`#ifdef CPU_MCXN947VDF`内で**A153ではシンボル自体が存在しない**）の3系統
- `boards.txt`は24プロパティ、`variants/<board>/`は`include`(25)/`linker`(1)/`src`(16)/`svd`(1)
- **`gdb-bridge`はボードごとにWindows exeが1本必要**（デバイス文字列を`-ldflags`で焼き込む方式）——`.bat`ラッパーがIDEのcortex-debugで動かないため。**3〜4枚目で破綻するので、そこで作り直す**必要があると手順書に明記した

**手順書に「罠」の節を設けた**——このプロジェクトが実際に代償を払った項目だけを列挙（ALT値の位置カウント方式が2ピンだけ外れる／ピンがテストポイントにしか出ていない／クロックが`mcu.cpp`と`clock_config.c`の2箇所またはどちらにも無い＝同じバグを3回踏んだ／`port_pin_config_t`の位置指定初期化がチップ間で壊れる／静的初期化順序の競合／SDKマクロ名の衝突／`*/`の早期終了）。**「コンパイルが通ったことを動作の証拠にしない」**——ソース配布移行時に114サンプル×2ボードが通ってから実機初回でハングした件——も立ち上げ順序の節に入れた。

**記述の裏取り**: 手順書に書いた具体的主張（A153のLPI2Cは1系統／A153に`Wire2`シンボルが無い／`io.h`の`PWM0`/`PWM1`の`#undef`／`arduino_tone.cpp`の分周シンボル差異）は、CLAUDE.mdの記憶ではなく**実ファイルをgrepして4件とも確認**してから確定させた。内部リンク4本も実在確認済み。

**READMEからの参照は他3ガイドと別の一文にした**——`docs/`の既存3本はコアの**利用者**向け、この手順書はコアを**拡張する側**向けで読者が違うため、同じ列挙に混ぜていない。`TUTORIAL.md`の「Where to go next」には入れていない（チュートリアル読者の次の一歩ではないため）。
...
**権威ある情報源はZephyrのpinctrlヘッダ**（NXP自身のデータから生成されたシリコン正確な定義、`#define <SIGNAL>_P<port>_<pin>  A15X_MUX('<port>',<pin>,<mux>)`形式）。**これは2つのcheckoutに存在する**——`/Users/tedd/dev/ArduinoCore/modules/hal/nxp/dts/nxp/mcx/`と`/Users/tedd/dev/slint-zephyr/modules/hal/nxp/dts/nxp/mcx/`。**私は片方（slint-zephyr）しか見ずに「A153のヘッダは無い」と結論し、1526ページのリファレンスマニュアルPDFを読む方向へ進みかけた**——ユーザーの「A153もZephyr内でN947と同様の定義があったのでは？」という指摘で誤りと判明。**存在しないと結論する前に、両方のcheckoutを確認すること**。指示されたPDF（`ref/MCXA1x2-1x3-RM.pdf`）は結果的に不要になった（そもそもこれは1044バイトのmacOS Aliasで、記録された参照先パスも失効していた——実体は`/Users/tedd/dev/MicroPython/_hardware_info/MCU/`に名前検索で発見）。

**監査結果**:

| ボード | 対象 | 結果 |
|---|---|---|
| N947 | 26ピン全部（PWM 6・ADC 4・I2C/I3C 6・SPI 4・Serial 6） | **全て正しい**——PWMのサブモジュール対応・ADCのA/B side channelマッピングまで含めて一致 |
| A153 | I2Cコンストラクタの4ピンペア | **実バグ2ペア**（下記） |
| C444 | I2Cの2ピンペア | 正しい（PTE0/PTE1 ALT6、PTC2/PTC1 ALT2、どちらも`I2C1_*`と一致） |
| A156 | I2Cの5ピンペア | 正しい（0.7の先行確認も兼ねた） |

**A153のI2Cコンストラクタの潜在バグ**:

| ピンペア | 正しいALT | 修正前 | |
|---|---|---|---|
| `I2C_SDA/SCL` (D18/D19 = P1_8/P1_9) | 3 | 3 | 正しい——**Wireが実際に通る唯一の経路** |
| `MB_MOSI/MB_SCK` (P1_0/P1_1) | 3 | 3 | 正しい |
| `I3C_SDA/SCL` (P0_16/P0_17) | 2 | 3 | **誤り**——ALT3は`LPSPI0_PCS2/PCS3` |
| `MB_SDA/MB_SCL` (P3_28/P3_27) | 2 | 3 | **誤り**——ALT3には何も割り当てが無い |

**なぜA153だけがこの形だったのか（上流の履歴を追って確定）**: **このコードはこのリポジトリで書かれたものではない**。上流r01lib（`/Users/tedd/dev/mcuxpresso/r01lib`）の**最初のコミット`6481a96`（2024-04-03「1st commit」）**に既にこの形で存在し、`717da18`で丸ごと取り込まれたもの。そのコミットでは**N947側も全く同じ形**（`if (...) ; else panic(...)` ＋ `constexpr int mux_setting = 2;`）——つまり**当時の設計そのもの**で、if/elseチェーンは「使えるピンの組み合わせのホワイトリスト（不正なら`panic`）」、ALTは別途1つの定数、という役割分担だった。**ペアごとのALT値が存在したのを畳み込んだのではなく、最初から存在しなかった**。

ペアごとの形に変わったのは`b832404`（2024-12-08「FRDM-MCXA156 support added」）で、理由は差分に明確に出ている——**A156は`LPI2C0`/`LPI2C1`/`LPI2C3`と複数インスタンスを持つため`unit_base`をペアごとに変える必要があり**、枝に中身ができたのでALTも一緒に入った（同じコミットで`LPI2C_MasterInit(EXAMPLE_I2C_MASTER,...)`→`(unit_base,...)`にも変わっている）。**A153はLPI2Cが1系統しかないので、枝の中身を開ける動機が構造的に一度も発生しなかった**——これが取り残された理由。

したがって真の欠陥は「潰したこと」ではなく、**4ペアをホワイトリストに載せた時点で「この4つは全部同じALTか」を確認しなかったこと**。当時実際に使われていたのはD18/D19だけで、そこはALT3で正しかったため、検証する機会が来なかった。

**修正**: ペアごとに`mux_setting`を代入する形（他ボードと同じ）に書き換え。**`mux_setting`を初期化子なしで宣言すると`-Wmaybe-uninitialized`が出る**——`panic()`が`noreturn`でないためelse枝のfall-throughをコンパイラが排除できない。既存のA156分岐が既にそうしているのと同じく、宣言時に初期化して解消した。ついでにC444分岐のpanicメッセージがA153からのコピペのままだったのも訂正（**文字列のみ、機能値は一切触れていない**）。

**この監査で何が実証されたか**: 「ALT値の位置カウント方式は信用しない」という教訓が既にドキュメント化されていても、**過去に書かれたコードを遡って洗い直さない限り、既に入っている誤りは残り続ける**。今回のバグは3件の既知ALTバグより古い時期の書き方で、Wireの経路が1本しか使われていないために5バージョン以上気づかれずに残っていた。

### ALT監査の完了（A153の残り全数）と、同型の潜在バグ1件の修正
「このほか0.6でやっておくべきことは？」への回答として、まず**A153で未監査だった残り**をZephyrヘッダで洗った。結果は**全て正しい**:

| 対象 | 結果 |
|---|---|
| PWM `P3_6`〜`P3_11` | ALT5 ✓。**サブモジュール/チャンネル対応も6本全て一致**（`P3_6=PWM0_A0`→sm0/chA … `P3_11=PWM0_B2`→sm2/chB） |
| `Serial` D0/D1 (`P1_4`/`P1_5`) | LPUART2 ALT3 ✓ |
| `Serial` MB_TX/MB_RX (`P3_15`/`P3_14`) | LPUART2 ALT2 ✓ |
| ADC ALT0 / I3C ALT10 | 実機検証済みの経路（`r01lib_I3C.ino`・analogRead精度確認） |

副産物として`MB_SDA`/`MB_SCL`=`P3_28`/`P3_27`=`LPI2C0` **ALT2**、`P0_16`/`P0_17`=`LPSPI0_PCS2/PCS3` **ALT3**も独立に再確認——直前のi2c.cpp修正の裏付けになった。

**記録の訂正**: CLAUDE.mdは「A153のMikroBus `Serial1`はLPUART2競合のため**物理的に不可能**」と書いてきたが、`Serial.cpp`のA153 `s_pinMap[]`には`{ MB_TX, MB_RX, LPUART2, ALT2 }`エントリが**実在**し、`Serial(MB_TX, MB_RX)`は解決する。正しくは「**構築はできるが、D0/D1の`Serial1`と同じLPUART2なので共存できない**」——だから`arduino_serial.cpp`がD0/D1側だけをグローバルに置いている。値自体は上表のとおり正しい。

**見つかった潜在バグ（修正済み）**: `r01lib_spi.cpp`の`cs_manual_control()`が、CSをLPSPIのハードウェアPCS機能へ渡す際のALTを**`2`決め打ち**にしていた。`mux_setting`はN947のMikroBusで**3**になる（`P3_23`=`FC6_P3`=ALT3）のに追従していない——**A153のI2Cと全く同じ「ペアごとに違う値を定数1つで済ませた」形**。

- **今日は到達不能**: `cs_manual_control()`の呼び出し元はコンストラクタ1箇所のみで、常に`true`（＝ALT0、プレーンGPIO）を渡す。`false`を渡すコードはリポジトリ全体に存在しない（`grep`で確認）。つまり誤った定数が実際に使われたことは一度もない
- **修正**: コンストラクタが実際に使ったALTを`peripheral_mux`メンバに記録し、`cs_manual_control()`はそれを復元する。C444分岐の`cs_manual_control()`は元から`pin_mux()`を触らない（C444はCSをハードウェアPCSにしない）ため変更不要——ただし対称性のためC444コンストラクタでも`peripheral_mux`は初期化した
- 両ボードで`--warnings all`警告ゼロ、`hello_world`のサイズは**完全に不変**。SPIを通す全13サンプル×両ボードをコンパイル確認（失敗は既知の3件のみ: `test_Wire2_frequency_accuracy_N947`のA153＝N947専用の設計どおり、`test_combined_peripherals`と`release_check/21`の外部`P3T1755.h`欠落＝CIスタブを与えれば両ボードとも警告ゼロで通ることを確認）

**この2件で言えること**: 「ペアごとに違う値を1つの定数に畳む」形は、**その定数が実際に使われる経路が1本しかないときに生き残る**。A153のI2CはWireがD18/D19しか通らないため、`cs_manual_control`はそもそも`false`が呼ばれないため。**到達不能なパスは、動かして見つかることが原理的にない**——先回りの全数監査でしか出てこない。

### 追加項目完了: `docs/api/`の陳腐化をCIで検出（6つ目のhygieneチェック）
リリース準備チェックリストの項目3（Doxygen再生成）を**忘れて2回踏んだ**（v0.4.0＝mcxPinState/バージョンマクロ追加分が丸ごと欠落、v0.4.1＝`mcu.cpp`のクロスリファレンス行番号がずれたまま）ので、機械化した。

**タイムスタンプではなくコミットで比較する**——`git clone`直後は全ファイルのmtimeが同一になるため、mtimeでは何も分からない。判定は既存の`mcxPinState`照合と同じイディオム:

```
docs_commit = git log -1 --format=%H -- docs/api
git log --oneline <docs_commit>..HEAD -- <DoxygenのINPUT> Doxyfile
```

空でなければ「docs/apiを最後に生成した後にソースが動いた」＝陳腐化。**再生成とソース変更を同じコミットに入れる通常の運用では`docs_commit`がそのコミット自身になり、`docs_commit..HEAD`から除外されるので誤検出しない**。

- **監視対象は`Doxyfile`の`INPUT`タグから読む**（ハードコードしない）——Doxygenの入力先を変えたときにチェックだけ古いディレクトリを見続ける事故を防ぐ。`Doxyfile`自身も監視対象に含めた（設定を変えれば出力も変わるため）
- **`--release`ゲート**: 開発サイクル中は生成物が遅れているのが正常なので、`changelog-heading`/`package-index-entry`と同じく`main`とタグでのみ有効
- **`SOURCE_BROWSER=YES`なのでコメントだけの編集でも出力が動く**——だから「ソースが変わった＝要再生成」で正しい

**CIのcheckout深さ**: コミット比較はshallow cloneでは答えが出ない。かといってこのリポジトリの履歴は**約145MB**（`variants/frdm_mcxa153/lib/*.a`の66MBが今も履歴に残っている）ので、毎pushで全取得するのは割に合わない。`hygiene`ジョブのcheckoutを**リリースセットが走るときだけ`fetch-depth: 0`**にする条件式を入れた。`update_package_index.yml`は元から`--release`を無条件実行するので`fetch-depth: 0`固定。

- **GitHub式の罠**: `cond && 0 || 1`は**常に1**になる（式評価で裸の`0`はfalsy）。`'0'`/`'1'`と文字列にして回避——ワークフローにもコメントで明記した
- shallow cloneで`--release`を走らせた場合は**黙ってスキップせず`fail`**にした（「検証できなかった」ことこそ、このチェックが防ごうとしている見落としそのものなので）

**検証（両方向を確認）**:
- 現在の`0.6.0-dev`（i2c.cpp/r01lib_spi.cppを触った後、Doxygen未再生成）→ **正しく落ち**、原因コミット4件を名前付きで表示
...
**症状**: `analogWrite(D9, 128)` は `PwmOut` のコンストラクタに入り、`resolve_pin()` が `s_pins[]` に無いピンで `_pin = -1` を返し、`panic( "PwmOut: unsupported PWM pin" )` ——**SOS点滅でスケッチが死ぬ**。`analogWriteFrequency()` も同じ経路。

**影響範囲が広い理由**（ALT監査の副産物として確定した事実）:

| ボード | D0〜D13 でFlexPWMに繋がるピン |
|---|---|
| A153 | **D3(P3_0)・D7(P3_1) のみ**。しかも`PWM0_A0`/`PWM0_B0`＝既存`PWM5`/`PWM4`(P3_6/P3_7)と**同じチャンネルの別ルート**で併用不可 |
| N947 | **ゼロ**（D0〜D13のどれにもFlexPWM代替が存在しない） |

つまり`analogWrite(9, ...)`のような**古典的Arduinoの書き方は、このボードでは原理的にPWMになりえない**。にもかかわらず、それが「無反応」ではなく「起動即SOS」だった。

**本家2実装を一次情報で確認**（どちらも致命的にはしない、が挙動は違う）:
- **AVR** (`wiring_analog.c`): `pinMode(pin, OUTPUT)`のうえ、タイマーの無いピンは`case NOT_ON_TIMER: if (val < 128) digitalWrite(LOW) else HIGH`
- **UNO R4 / renesas** (`analog.cpp`): `PwmOut::begin()`失敗時に`ptr = nullptr`とし、`if (ptr != nullptr)`で**黙って何もしない**

**採用したのはAVR流**——観測可能な方が「PWMになっていない」と即座に分かるため。

**実装で外しやすかった点**:
1. **`pinMode()`/`digitalWrite()`は自分でリナンバリングする**。`analogWrite()`内では既に生ピン値なので、そのまま渡すと**二重変換**になる。変換前の引数を`arduino_pin`に退避して渡すようにした
2. **このコアは`pinMode()`を呼ばないと`digitalWrite()`が効かない**（起動時ALT固定ピン対策でALT0へ再muxする実装）ので、AVRと同じくフォールバック時に`pinMode(OUTPUT)`が必須
3. **しきい値は128固定ではなく`(max_value + 1) / 2`**。`analogWriteResolution()`で1〜16bitに変わるため、12bitで`analogWrite(pin, 2000)`が消灯扱いになるのは誤り
...
- **TCPサーバーモード**: `-c "gdb_port 50077" ... -s DIR -f a153.cfg`で起動→**cortex-debugの正規表現が待つ`Info : Listening on port 50077 for gdb connections`を出力**→gdbを`target extended-remote localhost:50077`で接続→`load`→ブレークポイント到達まで確認。**【訂正】ここに当初「IDEと同じ引数の並び」と書いたが誤り**——`-s`に渡す値が実物と違っており、それが下記のバグを隠していた
...
### 実機バグ発見・修正: IDEからは`.cfg`を見つけられなかった（board-agnostic化の詰め）
上記の検証を通した直後、ユーザーがArduino IDEの「デバッグ」ボタンを押したところ**起動に失敗**（`OpenOCD: GDB Server Quit Unexpectedly`）。gdb-serverタブのログに`gdb-bridge: could not determine which board to debug`が出ていた。

**IDEが実際に渡してくる引数が、こちらの想定と違っていた**（ログから逐語で判明）:

```
launch.sh -c "gdb_port 50000" -c "tcl_port 50001" -c "telnet_port 50002"
  -s /private/var/.../Blink                                   ← スケッチのビルドディレクトリ
  -f ".../cortex-debug/extension/support/openocd-helpers.tcl" ← IDE自身のヘルパーが先頭に入る
  -f a153.cfg                                                 ← 裸のファイル名
```

つまり**cortex-debugは`-s`を自分のスケッチビルドディレクトリで上書きし、自前のヘルパースクリプトを先頭に差し込む**。`boards.txt`側の`debug.server.openocd.scripts_dir`は**そのまま渡ってこない**。`deviceFromScripts()`は`-s`とだけ結合していたので、`a153.cfg`に永遠に辿り着けなかった。

**修正**: スクリプトの探索先を3段にした——(1)`-s`との結合（arduino-cliはここで当たる）、(2)そのまま（絶対パス・cwd相対）、(3)**このバイナリ自身のディレクトリ**。`.cfg`はバイナリと同じ場所に置くと決めてある＝**このリポジトリが管理しているレイアウト**なので、IDEが`-s`に何を入れてこようと影響を受けない。エラーメッセージにも探索したディレクトリを出すようにした。

**なぜ私の検証をすり抜けたか（testing gapとして記録）**: 私は`-s`に**gdb-bridgeのディレクトリ**を渡して試していた。CLAUDE.mdに記録されていた引数の形（0.4.0時点の調査メモ）をなぞったもので、**実物のIDEが送る形ではなかった**。当時のgdb-bridgeは引数を一切読まなかったので`-s`が何であっても動き、記録が実物とずれていても誰も困らなかった——**引数を読むように変えた瞬間に、その古いメモが有害になった**。決定打はユーザーが貼ったIDEのログそのもので、そこに答えが全部書いてあった。**再現用の引数はドキュメントの記憶からではなく、実物のログから取ること。**
...
### `release_check/13`にmcxRCServoのパルス幅チェックを追加（実機未確認）
ユーザーから「同梱したmcxRCServoもリリースチェックすべき？サーボ無しで波形だけ見るなど．でもPWM機能のチェックでカバーできていて冗長では？」という問い。**冗長という見立ては8割正しかった**——実ファイルを読んで確定した結論:

| | 既存カバー |
|---|---|
| ピンから波形が出る／50Hzが出せる | `02`が完全にカバー。**目視を足す意味はない** |
| `analogWriteResolution(16)`を**実PWMピン**に掛ける | **どこにも無い** |
| duty 2.5%〜12%という低duty域 | **どこにも無い**（`02`は10〜90%と50%固定だけ） |

`01`の`analogWriteResolution(12)`チェックは**非PWMピンのフォールバック経路**（`digitalWrite`のしきい値判定）で走っており、FlexPWMには一度も届いていない。一方`mcxRCServo`のコンストラクタは`analogWriteFrequency(pin, 50)`＋`analogWriteResolution(16)`で、`position()`が投げるのはSG90なら**1638〜7864 / 65535**（20ms周期の0.5〜2.4ms）。**この組み合わせが壊れても静か**——コンパイルは通りCIも緑、panicも出ず、サーボが違う角度で止まるだけ。同梱した以上ここは本リポジトリの責任範囲になった。

**目視ではなく自動判定にした**。`PWM0`→`D8`をジャンパして**自分の出力を`pulseIn()`で測る**——サーボもロジアナも要らない。`13`は既に`pulseIn`グループ（`tone`の1kHzを測っている）なので道具立てがそのまま使え、**書き込み・確認サイクルを増やさずに済む**のが`13`を選んだ理由。期待値はSG90のデータシート値（0.5/1.45/2.4ms）から直接出るので、コア側のduty換算が変わっても期待値は正しいまま。窓は±50us——8bitのまま解釈されれば duty≥1.0 で全く別の値になるので確実に捕まる。

**ついでに見つけた設計上の注意**: `analogWriteResolution()`は**ピン単位ではなくスケッチ全体のstatic**。つまり`SG90 servo(PWM0);`を構築した時点で、そのスケッチの**以降の全`analogWrite()`が16bit解釈に変わる**。`13`では測定後に`analogWriteResolution(8)`／`analogWriteFrequency(PWM0, 1000)`へ明示的に戻し、その理由もコメントに残した。ライブラリのAPI設計上の性質であってバグではないが、サーボと他のPWMを同居させると後者が黙って壊れる。

**ヘッダの番号ズレも全数直した**: `13`の冒頭が`Release check 6/N`と**旧番号のまま**だった（`0n`/`1n`/`2n`へ振り直したとき、後から書いた`04`/`05`/`21`〜`23`だけが新方式`Release check NN:`で、先に書かれていた分が`N/N`のまま取り残されていた）。**`01`/`02`/`03`/`11`/`12`も同じ状態**だったので、ユーザー指示（「いま直して」）でまとめて新方式に統一——**11本すべてが`Release check NN:`（2桁ゼロ埋め）**になった。ヘッダのコメントと`Serial.println()`のバナー行の両方が対象。

あわせて**本文中の`#N`クロス参照もゼロ埋めに正規化**（`#2`→`#02`、`#1`→`#01`。`#12`/`#13`は既に2桁なのでそのまま）。ディレクトリ名は`01_`/`13_`と2桁なのに参照だけ`#2`と1桁で、**どのスケッチを指しているか一目で結びつかなかった**のを解消したもの。対象は`README.md`と`13`のヘッダ。

**旧番号のまま放置すると何が起きるか**: このリポジトリの実機検証記録は番号で書かれている。CLAUDE.md内には既に「新旧対応表」（`0n`/`1n`/`2n`への振り直し時に作成）があるが、**スケッチ自身が名乗る番号がディレクトリ名と食い違っている**状態だと、実機で走らせたときシリアルに出るバナーとチェックリストの項目が一致せず、「どれを実行したか」の記録が取れない。

**検証**: 両ボード`--warnings all`で警告ゼロ（A153 65004B / N947 67948B）、hygiene全項目パス（新設したコメントブロックの`*/`早期終了チェック含む）。**`--library`指定なしでコンパイルが通った**ことで、同梱プラットフォームライブラリが`release_check/`配下のスケッチからも正しく解決されることも確認できた。

**実機確認完了（2026-09-11、両ボード`ALL OK`）**——`PWM0`-`D8`の5本目のジャンパを足して実行:

| `position()` | 期待 | A153実測 | N947実測 |
|---|---|---|---|
| `0.0` | 500us | 498us | 498us |
| `0.5` | 1450us | 1449us | 1449us |
| `1.0` | 2400us | 2398us | 2398us |

**±50usの窓に対して2us以内**。3点とも同じ約2usの不足なのは`pulseIn()`の検出ループのオーバーヘッド（同じスケッチ内の1kHz toneも498〜499usと同傾向）で、系統誤差。

**両ボードで値が1us単位まで一致した**のが収穫: A153は`PWM0`=`P3_11`/**FlexPWM0**/コア96MHz、N947は`P2_3`/**FlexPWM1**/コア150MHzで、ペリフェラルもサブモジュールもプリスケーラ選択も別物なのに同じパルス幅が出た——`PwmOut::apply()`の分周探索が両チップで正しく一般化していることの裏付けになる。**16bit分解能＋50Hz＋duty 2.5〜12%という、`release_check`の他のどこも踏んでいなかった経路**が実機で通ったことが本題。既存のshiftOut/shiftIn/pulseIn/randomも全てOKで、5本目のジャンパが他の4本に影響しないことも確認。

**再検証（2026-09-12）**: ユーザーから「PWM0とD8のショートって実際にはやっていない気がする」と、上記2026-09-11の検証時に本当にこの5本目のジャンパが挿さっていたかへの疑義が出た。CLAUDE.mdの記録を無条件に信用せず、両ボードともユーザーに明示的にPWM0-D8ジャンパの接続を確認してもらった上で`13`を再度実機フラッシュ・確認——**N947: 499/1449/2399us、A153: 498/1449/2399us**（いずれも期待値500/1450/2400usに対し±2us以内）と、前回の値とほぼ同一の結果を再現。これでジャンパが実際に効いた状態でのパルス幅一致であることが再確認され、疑義は解消（前回の記録も正しかったと確定）。

### 判明: A153はMikroBusとArduinoヘッダが6組のMCUピンを共有している（N947は共有ゼロ）
「`02`のGPIOウォークをロジアナ目視ではなくループバックで自動判定にしたい、配線はシールド基板のジグで用意する」というユーザー提案の設計中に判明。**ジグ自体は0.6.0では見送り**（`02`のウォークをそのまま使う）だが、**ハードウェアの事実としてジグとは独立に効く**ので記録する。

**`io.h`をパースして機械的に確定させた結果**（目視ではなく、名前→生ピンまで別名解決してから重複を数えた）:
...
**A153: 34名 → 実体28ピン（別名6組）。N947: 34名 → 31ピン、別名ゼロ**（`A0`/`A1`/`MB_AN`が`DISABLED_PIN`）。**2ボードは電気的トポロジが別物**で、ピン名で書いた配線は同じ結線にならない。

**効いてくる先**:
- **現在の`02`のウォークはA153で6ピンを二度踏んでいる**（実害はないが、カバレッジの数え方が名前ベースだと過大になる）
- ピン同士を短絡するジグは**MCUのネットリストで設計しないと多重ネットになる**。実際、隣接ペア案をA153に当てると`D6-D7`＋`MB_AN-MB_RST`＋`A4-A5`が`D7≡MB_RST`・`MB_AN≡A5`経由で**1本の4ピンネットに融合**し、`D4-D5`と`MB_PWM-MB_INT`・`D8-D9`と`MB_RX-MB_TX`は**同じ2ピンを2度結ぶ冗長配線**になる
- **0.7でA156を足すときは、同じ別名チェックを最初にやること**。`docs/porting_a_new_board.md`が挙げる「手で触る箇所」に、この観点はまだ入っていない

**この過程でやらかした読み違い（再発防止）**: `io.h`のボードブロックを取り違え、**A156のブロックをA153だと思って読んだ**。`CPU_MCXA153VLH`は583行目からで、私が読んでいたのは`CPU_MCXA156VLL`（437行目〜）だった。これを根拠に「A153の`A5`は`P1_13`、`SW2`は`P1_7`で別ピン」「A153は34ピン全部テストできる」とユーザーに報告してしまい、あとで全部訂正することになった。**`io.h`は5ボード分のブロックが1ファイルに並んでいるので、`sed -n 'N,Mp'`で読む前に必ず`#elif CPU_*`の位置を確認して範囲を確定させること。** 正しいA153の値は `A5 = P3_30`（= `MB_AN`）、`SW2 = P3_29`、`SW3 = P1_7`、`A4`/`A5`は`P3_31`/`P3_30`で**ADC非対応（デジタル専用）**。なお`PIN_MAPPING_A153.md`の記述（`D18`=`P1_8`等）は**正しかった**——誤っていたのは私の読み取りの方。

**ジグ設計で未解決のまま止めた点**（再開するときの続き）: (1) A153の`Wire1`(I3C)は`P0_16`/`P0_17`でArduinoヘッダにもウォーク対象にも入っていない独立ピンのため、ロジアナ用コネクタから届くか`ref/FRDM-MCXA153.pdf`で要確認、(2) N947は31ピン＝奇数なので必ず1本余る（どれを余らせるかは自由配線なら選べる）、(3) 共通PCB＋ボード別ソルダージャンパで賄えるか。

**あわせて`img/`のピン図2枚を訂正**（ユーザー作業）: `D18`/`D19`と`I2C_SDA`/`I2C_SCL`の対応が**両図とも逆**だった。`io.h`（`I2C_SDA = D18` / `I2C_SCL = D19`、両ボード共通）と`PIN_MAPPING_*.md`の表が正で、図だけが誤っていた。**配線するとき最初に見るのは図**なので影響が大きい。

### GPIOループバックジグ（設計確定・**実装は0.7.0-devで再開する**）
「`02`のGPIOウォークをロジアナ目視ではなくループバックで自動判定にしたい、配線はシールド基板のジグで用意する」というユーザー提案。**0.6.0は現状の`02`のウォークのまま出す**と決めたうえで、設計だけ先に確定させたもの。

**再開の仕方**: 設計・配線仕様・検証器はすべて`.github/scripts/check_jig_pairing.py`に入っており、**このファイルは`0.6.0-dev`にマージ済み＝作業ツリーにある**。`python3 .github/scripts/check_jig_pairing.py --table`が両ボードの配線仕様（ジグA/B、ロジアナのCH割り当て、載せるデバイス）をそのまま出力する。作業用の`gpio-loopback-jig`ブランチは**固有コミットが無い状態（完全にマージ済み）だったので削除した**——実装は`0.7.0-dev`を作ってからそこで進める。0.7はA156追加のサイクルなので`release_check`側も動く見込みで、`31`や`02`分割はその上で書いた方が整合する。

**なぜウォークより良いか**: 現在のウォークは**出力しか見ていない**（LAで波形が出るのを人が見る）。ループバックなら**全ピンの入力側も同時に検証**され、しかも自動判定になる。GPIOの入力側はいまどのリリースチェックでも踏まれていない。

**`02`のウォークは置き換えるしかない（併存できない）**: ウォークは**全ピンを`OUTPUT LOW`にしてから1本ずつ`HIGH`にする**ので、ショートされた相方は`OUTPUT LOW`のまま。**HIGHドライバとLOWドライバが150ms×34ピン分ぶつかり続ける**。「ウォークを残してジグを付けっぱなし」は選択肢にならない。ただし**ジグが無い環境の代替手段としては残す**（ユーザー判断）。

**決めたこと**:
- **ジグはボード別でよい**（ユーザー判断）。これで共通PCBの制約が外れ、両ボードとも解けた
- **ナンバリング**: `02`を分割して`02_manual_observe`（PWM/analogWriteFrequency/tone/SDKトグル＝ジグと無関係に常に目視）と**`09_gpio_walk_no_jig`**（ウォークのみ、`31`の代替と名前で明示）に。ジグ必須の新グループ**`3n`**を追加し**`31_gpio_loopback`**を新設
- **ロジアナ用コネクタも載せる**（ユーザー追加要望）。Logic16の16chにちょうど収まる: CH0-3 `SPI`、CH4-7 `SPI1`、CH8-9 `Wire`、CH10-11 `Wire2`、CH12-13 `Wire1`、CH14-15予備。**このプロジェクトのバグ履歴と正面から噛み合う**——N947の`SPI`が24MHz要求で実測31kHz、`SPI1`が半分、I2Cの`FILTSDA`残留、いずれもロジアナでしか出なかった

**ペア表は`.github/scripts/check_jig_pairing.py`に置いた**（`hardware/nxp/mcx/`の外なのでリリースzipには入らない）。**別ドキュメントにしなかったのは、仕様とそれを検証するモデルがズレるのを防ぐため**——`--table`で配線仕様として出力できる。A153は14本・余りゼロ、N947は15本＋`MB_SCK`が余り（19本すべてが余り候補として成立するので変更可。`MB_SCK`にしたのは`12`のSPI1ループバックがクロックとして駆動する＝出力としての機能カバーが別途あるため）。

**手作業では2回間違えた。これが検証器を書いた理由**:
1. **`MB_CS`-`A2`を組んでしまった**——`21`は**SPI1（`MB_CS`を駆動）と`analogRead(A2)`を同じループで**やるので、A2がCSの電位を読む。31ネット×10スケッチを目視で追うと確実に取りこぼす
2. **最初のペア表をArduinoのピン名で組んだ**——A153では6組が同じシリコン（上のセクション参照）なので無意味。MCUネット単位でしか設計できない

**モデルが知らないこと（重要）**: 検証器は「誰が駆動し誰が読むか」しか見ておらず、**容量負荷・ドライブ能力・配線長を知らない**。実際、**N947の`A5`（10kΩプルアップ＋100nFがGNDへ）を`MB_RX`（I3C SDA）にぶら下げる解を平気で出した**。`CAP_LOADED`として手で足したが、**同種の未発見のピンがあれば同じように見逃す**——図と`ref/`の回路図が唯一の情報源。

**併せて分かったこと**: `BUZZER_PIN = D13` で、**toneのピンがSPI SCLKと同じ**。`21`はtoneとSPI1とanalogReadを同時に回すので、`D13`の相方は`21`実行中も静かでなければならない。

**ジグは2枚に分ける（ユーザー判断）**——ループバック用（**ジグA**）とI2C/I3Cデバイス用（**ジグB**）で、差し替えて使う。スタックさせない。**デバイスをジグAに載せると上の表が作り直しになる**のが理由: デバイスがバスにぶら下がっていると、そのバスを触るどのスケッチでもラインが生きるので、バスのピンの相方は「そのバスを使わないスケッチでは静か」では足りず**どのスケッチでも駆動されず読まれもしない**ピンでなければならなくなる。この制約で解くと**N947は最低3ピン余る**（`D10`/`D13`/`D18`）。分ければカバレッジを一切落とさずに済む。

**ロジアナのタップも同じ理屈で2枚に分けた**——どちらを挿しているかで走るスケッチが違うので、ジグA＝`SPI`/`SPI1`の8ch（`12`のとき）、ジグB＝`Wire`/`Wire1`/`Wire2`の6ch（`22`/`01`/`05`のとき）。1枚に14ch詰める必要がなくなった。ジグBに載る実デバイスは**LM75系センサー1個（`D18`/`D19`）だけ**——`Wire1`のセンサーは両ボードともオンボード、`Wire2`はデバイス不要のバススキャンなので。

**A153の`Wire1`のタップ位置は解決済み**: `P0_16`/`P0_17`はArduinoヘッダに出ていないが、回路図（`ref/FRDM-MCXA153.pdf` p7のI3Cセンサー部）が**`J20`/`J21`**として引き出している（`P1_11/I3C0_PUR-P3T`も同様）。Arduinoヘッダのフットプリント外なので、**ジグBからは積層ではなく飛び線で取る**。N947は`Wire1`が`MB_RX`/`MB_TX`＝MikroBusコネクタから直接取れる。あわせて**A153には`Wire2`が存在しない**（LPI2Cが1系統）ので、`MB_SDA`/`MB_SCL`のタップはA153では無意味——名前は解決するのにタップとしては死んでいる、という罠。この2点は`--table`が自動で注記する。
...
2026-09-12、Linuxで A153・N947 両ボードともIDE内蔵デバッガの動作を確認（`0.6.0-rc2`ステージング経由）。ここまでで:
...
- **A2 `13`のmcxRCServoパルス幅** / **A3 FlexComm5実測** / **B1クロック監査の実行時検証化** / **B2 LPI2Cインスタンス別クロック**: いずれも完了済み（各節参照）

**【訂正】直後に一度「これで0.6.0の実機検証は全完了」と書いたが誤り**。上の4項目は**今回追加・変更した機能の差分だけ**の確認で、**チェックリスト項目7（`examples/release_check/`を`01`〜`23`全グループ通しで実機スイープする）とは別物**。ユーザーの指摘で気づいた。
...
## v0.7.0（`0.7.0-dev` ブランチ、2026-09-26リリース）

2026-09-13、`0.7.0-dev`を作成して開発サイクルを開始（`74c1239`）。`platform.txt`の`version`系と`Doxyfile`の`PROJECT_NUMBER`を0.7.0に。`arduino_io.h`は0.6.0での照合以降変わっていなかったので、`MCXPINSTATE_VERIFIED_AGAINST`も同時に上げた（上流の`mcxPinState`リポジトリにも同じ変更）。**ただしこのサイクル中に`arduino_io.h`は`de27445`（`SDA`/`SCL`の追加）で変わっている**ので、リリース前にピン表との照合をやり直す必要がある。

同日、`CLAUDE.md`（579KB、約178kトークンで、毎セッション全文が読み込まれていた）から版ごとの作業記録をこのファイルへ切り出した（`96aea00`）。

### 方針の変更: 0.7.0はA156の追加ではなく、AVR/UNO互換の穴埋めになった
v0.6.0の節の「0.7・0.8の方針」では**0.7=FRDM-MCXA156の追加**としていた。実際の0.7.0の中身は、既存2ボードでArduinoのスケッチやライブラリがそのまま動かない箇所を埋める作業になった。`setWireTimeout()`、I2Cターゲット（スレーブ）モード、`Serial`のフレーム形式・`end()`・`serialEvent`、`Wire`の`Stream`化、AVR時代のヘルパー群、`EEPROM`、2枚同時接続での書き込み・デバッグ、など。

- **I2Cターゲットモードは「明示的に後回し」だったが0.7.0で実装した**。後回しの理由は「r01libにスレーブドライバが無く、低レベルから新規開発になる」だったが、実際にはSDKの`LPI2C_Slave*`割り込みAPIの上に載せられた。SDKが呼び出し側に任せている部分を埋める作業が中心だった（下記）
- **FRDM-MCXA156の対応とGPIOループバックジグは0.8以降**（2026-09-26、ユーザー判断）。ジグの設計は`.github/scripts/check_jig_pairing.py`にあるまま
- **上流`r01lib`への反映も保留**（2026-09-26、ユーザー判断）。コア同梱の`r01lib`は上流と21ファイル・約1,800行ずれている。上流はA156・N236・C444も対象にしており、それらはここでは確かめられない。**このコアで他のボードの対応が揃ってからまとめて反映する**

### `Wire.setWireTimeout()`（`21ac5e9`・`1d17585`・`3820bc9`）
LPI2Cのハードウェアのピン・ロー・タイムアウトの上に実装した。カウンタを有効にするだけでは足りず、実機で3件の追加修正が要った（CHANGELOG参照）: TX FIFO待ちのループがエラーを見ていなかった、バスビジーフラグがSTOPでしか解除されなかった（`BUSIDLE`で解除されるように）、タイムアウト後にマスタ自身がバスを握り続けた（STOPを積むように）。

**あとから見つかったバグ（`3820bc9`）**: `BUSIDLE`を有効にすると、有効化した直後のマスタは両ラインが一定時間（最大約0.34ms）Highなのを見るまでバスをビジーとみなす。そのため`begin()`・`setWireTimeout()`・`setClock()`の直後の転送が`132`で失敗した。**テストが見逃したのは、全転送の前に`Serial.flush()`があり、その待ちが0.34msを覆っていたから**。ターゲットモードとタイムアウトを組み合わせて試している途中で気づいた。テストは「各呼び出しの直後にすぐ転送する」ケースを加え、旧コアで失敗することを確認した。

- **最初の修正はタイムアウトを2.6〜6.9ms延ばしていた**。`wait_us(1)`でポーリングしていたが、`wait_us()`はdouble演算なので1回の呼び出しが1µsよりずっと長く、SCLが保持されている間もループし続けた。`BUSIDLE`の期間から算出した時間を1回`wait_us()`する形に変えた
- 同時に、N947の`Wire2`で同じテスト一式を初めて実機で通した（ジャンパ2本追加）。A153の400kHz・1MHzでの上限（約21.8ms・約10.9ms）も実測した
- CI: `workflow_dispatch`とpushが同じconcurrency groupで互いにキャンセルしていたので、groupに`github.event_name`を加えた

### 2枚同時接続での書き込み・デバッグ（`9678f48`・`f3d234c`）
2枚つなぐと、ポートを選んでいても書き込みが`Multiple probes detected`で失敗していた。書き込みは、ポートのUSBシリアル番号をLinkServerの`--probe`に渡すように直した。デバッグ時はArduino IDE 2がポートを渡さないので、`LinkServer probes`の`Device`列のチップ名でプローブを選ぶ（同じ種類の2枚は区別できず、エラーで止まる）。**macOSでしか確認していない**。Windows/Linuxはリリース前チェックリスト項目7に入れた。

この確認の途中で、ローカル検証の罠を2つ踏んだ（`2d771d2`、`CLAUDE.md`に記録済み）。Boards Managerで入れた`0.6.0`が`-dev`のsymlinkより優先されていたこと、arduino-cliのビルドキャッシュがボードを区別しないこと。

### `Serial`のフレーム形式・`end()`・`serialEvent`、`Wire`の`Stream`化（`3ab2cff`）
**ループバックではフレーム形式の誤りを検出できない**。受信側も自分の送信側と同じ形式を期待するからだ。そこで`test_Serial1_format_end_serialEvent`は、RXピンをGPIO入力レジスタ経由でビット単位に読み、12形式すべてのデータビット・パリティビット・ストップビットを確かめる。パリティの偶奇を入れ替える、2ストップビットを無視する、とわざと`format()`を壊すとテストが失敗することも確認した。

`Wire`の128バイトバッファは、`write()`が境界を確かめておらず、129バイト目が直後の変数（キュー済みバイト数）を上書きしていた。これもこのとき直した。

### `Wire.begin()`のオーバーロードと`panic()`のメッセージ（`1e87ca6`）
このコアの`begin(int)`は周波数を取っていた。そのため、ターゲット用に書かれた`Wire.begin(0x10)`がビルドは通り、16Hzのコントローラとして動いていた。

**これを`panic()`にしたところ、どの`panic()`のメッセージも誰にも届いていなかったことが分かった**。コアは`SDK_DEBUGCONSOLE=0`でビルドされるので`PRINTF`は`printf`になり、その出力はすべて捨てられていた。見えていたのはSOSの点滅だけだった。`panic()`はUARTをポーリングしてUSBシリアルへ直接書くように変えた。

### `SDA`/`SCL`（`de27445`）
`FXLS89xx_Arduino`（`pinMode(SDA, INPUT)`を使う）がビルドできないことから見つかった。**このとき一度、コンパイル結果をログの先頭のgrepで判定し、`SDA`のエラーを見落とした**。以後、コンパイルの成否は終了コードで判定する。

### I2Cターゲット（スレーブ）モード（`30ddc48`）
SDKの`LPI2C_Slave*`割り込みAPIの上に実装した。実機で見つかった、SDKが呼び出し側に任せている部分:

- A153の`fsl_lpi2c` 2.5.4はスレーブを有効にするときマスタを止める（N947の2.2.4は止めない）ので、以後のコントローラ転送が永久に待った
- `onRequest()`の返事より多く読まれると、SDKは何も送らずクロックストレッチでバスを握り続けた（AVRと同じく0xFFを送るようにした）
- ボード自身のターゲットを読むと先頭バイトが失われた（受信データを`requestFrom()`のバッファと分けた）

**N947の`Wire2`（LPI2C3）はターゲットとして一切応答しない。原因は不明**。コントローラとしては動き（`Wire2`をコントローラとして実機で試したのはこれが初めて）、スレーブ関係のレジスタも動作する`Wire`（LPI2C2）と一致した。`begin(address)`はそのバスでは理由を示して`panic()`する。

### `Wire.begin()`で内部プルアップを有効に（`810af15`）
**FRDM-MCXA153のD18/D19には外付けのプルアップ抵抗が無い**（N947にはある）。他に何もつながっていないバスは浮いており、`begin()`直後の転送が`132`になっていた。それまでターゲットモードのテストとサンプルは、PORTのPCRを直接書いてプルアップを有効にしていた。AVRの`twi_init()`とUNO R4の`begin()`がやっているのと同じく、`begin()`で有効にし、`end()`で無効に戻す形にした。

### `EEPROM`ライブラリ（`1f45833`）
**容量は1KB、A153はフラッシュ上位16KBを割く**（ユーザー判断: 「1KBで0.7.0に入れる．A153は16KB削っていい」）。1KBはUNO R3と同じ容量。読み出しをRAM上のコピーから行う設計なので、容量はそのままRAM使用量になる（A153のRAMは24KB）。

フラッシュの消去単位は両チップとも8KBセクタなので、A153は8KBの半分×2＝16KB、N947は第2バンクに64KB。書き込み単位は、A153が16バイトのフレーズ（ページ内の任意位置に書ける）、N947が128バイトのページ（`FLASH_Program`はアドレスが128バイト境界でないと`kStatus_FLASH_AlignmentError`＝status 101）。各フラッシュ操作のあとLPCACをクリアする。

**実機で確かめたこと**:
- `LinkServer flash load`（`upload.sh`が使う）と、`gdb-bridge`経由のgdbの`load`（IDEのDebugボタン）は、使っていないセクタを消去しない。前後でダンプして比較した
- 書き込み時間: 通常0.1〜0.5ms、コンパクション（もう片方の半分への移し替え）を伴う書き込みはA153で最大約5.6ms、N947で最大約10.7ms
- **シリアル受信の欠落**: 115200bpsで連続送信したとき、コンパクション1回あたりA153で約20バイト、N947で約60バイト失われた。A153は、実行中のプログラムと同じフラッシュを消去・書き込みする間は割り込みが待たされるため。**N947は「割り込みが止まらないので影響なし」と見込んでいたが、それは誤りだった**。割り込みは動いていても、書き込みがブロックしている間に64バイトのリングバッファがあふれる。欠落はすべてコンパクションの直後にあり、それ以外の書き込みでは失われていないことを、欠落を1件ずつ突き合わせて確かめた

**テスト自体のバグを3件出した**:
1. パターン領域に`float`の保存先が重なっていた
2. N947では使えない`analogRead(A0)`を呼んで`panic()`した
3. 常に真になる判定（`|| true`）が残っていた

対策として、アドレスの割り当てはスケッチ先頭の定数にまとめ、ボードごとのピンは`PIN_MAPPING_*.md`で確かめてから使う。
...
### `EEPROM`の書き込み中リセットのテストと、N947で起動ごとにハングする不具合の修正
それまでCHANGELOGには「書き込み中にリセットされても直前までのデータは残る（設計上。未試験）」と書いていた。これを確かめる`test_EEPROM_reset_during_write`を作った。
- **中身**: 4バイトのカウンタ8個を順に書き、それ以外のバイトは固定パターンにする。WWDT0でボードをリセットし、次の起動でカウンタが連続した8個の値になっているか、パターンが無傷かを確かめる
- **狙い方**: 書き込みの内側のランダムな時点を狙う。WWDTの残りカウント（`TV`）が狙った値になった瞬間に書き込みを始める
- **コンパクションの狙い方**: 半分の試行では、先にログを満杯にしてから狙う。満杯かどうかはテストがフラッシュを直接読んで調べる
- **切られた書き込みの記録**: どの値を書いている途中だったかは、`.noinit`のRAMでリセットをまたいで持ち越す
- **WWDTはレジスタを直接叩く**: coreにSDKドライバが無いため。両チップとも同じIPで、1MHz/4＝4µs刻み

**N947で本物の不具合が見つかった**: コンパクションの消去をリセットで途中で切ると、そのセクタがECC不一致の状態で残り、読むとバスフォールトになる。
- `start()`は両方の半分のヘッダを読むので、次の起動で最初に`EEPROM`へアクセスした時点でHardFaultしてハングした
- アップロードはこの領域を消さないので、以後はEEPROMを使うスケッチが**起動のたびに**ハングする。シリアルには空行1つしか出ない
- LinkServerのgdbで見ると、0x1F8000がデバッガからも読めなかった
- `FAULTMASK`＋`CCR.BFHFNMIGN`でバスフォールトを無視しながら読み、`CFSR`で読めたかを判定するスキャンをしたところ、そのセクタに読めない16バイト単位が67個あった

**修正**: `EEPROM.cpp`の`read_flash()`で、`start()`のフラッシュの読み出し（ヘッダ・イメージ・ログ）をすべてこの方式にした。
- ヘッダが読めない半分は無効として扱う
- 読めないログの記録は、CRC不一致と同じく飛ばす
- 割り込みを止めるのは、1回のコピー（最大128バイト）の間だけ

修正後、ハングした状態のN947にそのまま書き込むと、読めない単位を68個見つけたうえで正しく起動した。続く1000回の試行ではすべてOKで、そのうち245回の起動が読めないフラッシュを見つけ、乗り越えている。A153では修正前後の計2000回で、読めないフラッシュは一度も生じなかった。`release_check/06`も両ボードで2回（2回目はアップロードをまたいで）通った。

**テストハーネス側でも3件つまずいた**:
1. WWDTを起動した直後の`TV`は古い値を返すため、「カウンタが動いていない」と誤判定した。起動後1ms待つようにした
2. リセット要因にWWDTの`WDTOF`を使ったが、WWDTが起こしたリセット自体でこのフラグが消える。CMCの`SRS`（WWDT0ビット）で判定するようにした
3. 乱数の種を「次に書く値」だけから作ったため、ロールバックされると同じ値→同じ時点を狙い続けた（`x=57us`で473回）。試行回数も種に混ぜるようにした

加えて、「書き込み完了」の印をチェックサムの対象にしていたため、完了直後に来たリセットで状態の保存が途中で切れ、状態が失われることがあった。完了の印は1回のストアで書けるよう、チェックサムの外に出した。

**限界**: リセットは電源断ではない。電源断ではセルが書きかけで残りうるので、CHANGELOGの「電源断は未試験」はそのまま残している。1記録（A153は12バイト、N947は124バイト）を超える`put()`は、途中で切られると新旧が混ざる（`EEPROM.h`に記載済み）。
...
WindowsとLinuxのポート検出も、LinkServerと同じ形のシリアル番号を返している（保険の分岐には入らなかった）。
macOSでは、ユーザーがIDEで2枚つないだまま、各ボードのデバッグをもう一度試して問題なしと報告した（1枚ずつ順番に。2つのウインドウで同時にデバッグするのは試していない）。
Windows・Linuxでも、2枚つないだままの書き込み（ポートを切り替えてそれぞれ）とデバッグが問題なしと報告を受けた。**これでmacOS・Windows・Linuxの全てで、2枚接続での書き込みとデバッグを確認した**。

あわせて、0.6.0のときに削除し忘れていた`0.6.0-rc2`のプレリリースとタグを削除した（`0.6.0-rc1`はタグごと削除済みだった）。

確認が済んだので、`0.7.0-rc1`のプレリリースとタグ、`staging-0.7.0`ブランチを削除した。本番リリースでは改めてステージングを作って3プラットフォームで確かめる。
...
**6. 全サンプルのコンパイル**: 両ボードのfull tierで全部通った（A153 85本、N947 90本。`NUM_ANALOG_INPUTS`を直したあとに流し直した結果）。

**7. `release_check`（実機）**:
- `01`: 両ボード`ALL OK`（A153 110項目、N947 112項目）
- `04`: 両ボード`CONFLICT`/`MISMATCH`なし。ビルドで`mcxPinState`の`#warning`も出ない
- `06`: 1回目は両ボードで「前回のデータがLOST」の`1 FAILED`。前回の`06`のあと、IDE確認用のスケッチが0〜7番地に印を書いていたためで（`06`は0〜999番地の模様を見る）、`EEPROM`の不具合ではない。失敗した実行は完了の印を残さないので、次は通しで`ALL OK`、その次は前回のデータが書き込みをまたいで`kept (OK)`で`ALL OK`（両ボード）
- `07`: 両ボード`ALL OK`。A153は1000回で読めないフラッシュ0回、N947は270回の起動が読めないフラッシュを見つけ、失敗0
- `02`: 両ボードで、ユーザーがロジックアナライザ・スコープ・音で確認してOK（ピンの順番のHIGHはA153 34本・N947 31本、PWMの6チャンネルと周波数変更、`tone`、D2の方形波）。GPIOの切り替えは`digitalWrite()`がA153 1.27MHz・N947 2.05MHz、SDKのAPIがその約34倍
- `03`: 両ボードOK。SW2の3回がすべて数えられ、`detachInterrupt()`後は反応なし、LOWレベルの割り込みは押している間に大きく増えた
- `05`（N947）: ユーザーがロジックアナライザで`Wire2`のSTART・アドレス・STOPを確認してOK
- `11`・`12`・`13`: 両ボード`ALL OK`（配線1: D0-D1・D2-D3・D4-D5・D6-D7・PWM0-D8・D11-D12・MikroBusのMOSI-MISO、N947は加えて`MB_TX`-`MB_RX`。3本まとめて挿したままで流せた）
- `14`: 両ボード`ALL PASS`（A153 42項目、N947は`Wire2`の節を含めて77項目）。配線1を全部外してから挿す。N947の`MB_TX`-`MB_RX`は`Wire1`のピンなので、残すと`Wire1`の節を妨げる
- `24`: 両ボード`ALL OK`（それぞれ9項目）。D18-D18・D19-D19・GND-GNDでつなぎ、ビルドを済ませてから2枚に同時に書き込んだ（RESETの押し直しは不要だった）
- `21`（A153）: 40秒・211ループで`WARNING`0件。`Serial1`（D1-D0）は毎回前のループの`hb<番号>`を受け取り、`SPI1`は`0x1234`が返り、温度は約25.4℃。最初の試行ではジャンパの挿し間違いで`Serial1`が毎回空だった（配線を直して解消）
- `22`（A153）: P3T1035xUK-ARD（0x72）をD18/D19につなぎ、15秒で36回の読み出しがすべてOK（`requestFrom()`の`stop=false`/`true`とも）、25.000〜25.125℃
- `23`（A153）: `SDBitmapViewerDemo`（`Waveshare_TFT_Touch` 1.3.0、SD 1.3.0）で、ユーザーが画像の崩れ・速度とも問題なしと確認。1枚目は765ms
