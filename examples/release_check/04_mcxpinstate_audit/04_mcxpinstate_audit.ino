/** Release check 04: pin-ownership audit via the bundled mcxPinState library
 *
 *  A copy of that library's own CombinedPeripheralsAudit example, kept
 *  here so a release check covers the library this package now ships.
 *  Edit the original under
 *  hardware/nxp/mcx/libraries/mcxPinState/examples/, not this copy.
 *
 *  Pass condition: no "*** CONFLICT ***" and no "*** MISMATCH ***"
 *  anywhere in the two printed tables. A CONFLICT means two live objects
 *  claim the same physical pin; a MISMATCH means a pin's actual PORT MUX
 *  disagrees with what its owner asked for.
 *
