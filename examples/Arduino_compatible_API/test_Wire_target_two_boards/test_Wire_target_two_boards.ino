/** Wire target (slave) mode between two boards: an FRDM-MCXA153 and an
 *  FRDM-MCXN947, each a target on Wire and, in turn, the controller of
 *  the other.
 *
 ...
 *  controller, and tells it to go ahead; the FRDM-MCXN947 then runs the
 *  same checks the other way. Each board judges its own side: read
 *  "ALL OK" or "N FAILED" on each board's Serial Monitor.
 *
 *  The two runs have to meet: if one board is still running this sketch
 *  from before (its run over, its target still answering) when the other
 *  starts, the new run talks to the old one and the hand-back never
 *  comes. Press RESET on both boards, close together, to start clean.
 *
 ...
 *  stored from there, a read returns 4 registers from the pointer.
 *  Writing a pointer of 0x10 means "your turn".
 */
