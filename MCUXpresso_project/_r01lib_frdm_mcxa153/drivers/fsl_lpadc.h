...
/*
 * MSDK_REG_SECURE_ADDR() is used by LPADC_GetInstance() below but is only
 * defined in newer SDK generations' fsl_common_arm.h. This project's copy
 * predates it and no other vendored driver here needs it, so it's scoped to
 * this file rather than patching the shared fsl_common_arm.h. The #ifndef
 * makes this a no-op if fsl_common_arm.h is ever upgraded to define it.
 * This project does not build with TrustZone/CMSE, so identity is correct.
 */
